...
#Region "Api/constants"
    ...
    ''Dhruv setting the user scalability
    Const WM_CAP_SET_SCALE As Integer = WM_USER + 53
...
#End Region
    ...
    Public Function initCam(ByVal parentH As Integer, ByVal picOutputheight As Int32, ByVal picOutputwidth As Int32) As Boolean
        'Gets the handle and initiates camera setup
        If Me.iRunning = True Then
            MessageBox.Show("Camera Is Already Running", gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Return False
            Exit Function

        Else
            hHwnd = capCreateCaptureWindowA(iDevice, WS_VISIBLE Or WS_CHILD, 0, 0, OutputWidth, CShort(OutputHeight), parentH, parentH)

            If CBool(SendMessage(hHwnd, WM_CAP_DRIVER_CONNECT, CShort(iDevice), CStr(0))) Then
                ' 
                'Set the preview scale 
                ' 
                SendMessage(hHwnd, WM_CAP_SET_SCALE, CShort(True), CStr(0))

                ' 
                'Set the preview rate in milliseconds 
                ' 
                SendMessage(hHwnd, WM_CAP_SET_PREVIEWRATE, 66, CStr(0))

                ' 
                'Start previewing the image from the camera 
                ' 
                SendMessage(hHwnd, WM_CAP_SET_PREVIEW, CShort(True), CStr(0))

                ' 
                ' Resize window to fit in picturebox 
                ' 
                SetWindowPos(hHwnd, HWND_BOTTOM, 0, 0, picOutputwidth, picOutputheight, _
                                SWP_NOMOVE Or SWP_NOZORDER)
                If setCam() = False Then
                    MessageBox.Show("Camera Is Not Running. Please attach Camera Properly.", gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Return False
                End If
            End If
        End If
