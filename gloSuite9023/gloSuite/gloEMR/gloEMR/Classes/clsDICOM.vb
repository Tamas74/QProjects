...
#End Region
    ...
    Public Function GetDICOMDetails(ByVal nPatientID As Long, Optional ByVal TestID As Int64 = Nothing, Optional ByVal OrderID As Int64 = Nothing, Optional ByVal IsTest As Boolean = False, Optional ByVal sDicomIDs As String = Nothing) As DataTable
        ...
        Try
            ...
            '  strSQL = "SELECT DicomID, nPatientID, DICOMFileName, DICOMFileExtn, IsAckReview, AckRvwDescription, NotesDescription FROM DICOMdetails where nPatientID = " & nPatientID & ""
            'if this form opened from view->dicom then
            strSQL = "SELECT  DicomID, nPatientID, DICOMFileName, DICOMFileExtn, DICOMDocName, nUserID, dtImportDateTime FROM DICOMdetails where nPatientID = " & nPatientID & " Order By dtImportDateTime Desc"
    ...
    Public Function DeleteDICOMFile(ByVal DicomId As Int64, ByVal PatientID As Long)
        Dim objCon As New SqlConnection
        Dim objCmd As New SqlCommand
        Dim _result As Boolean = False
        ...
        Try
            ...
            'objCmd.CommandText = "DELETE  DicomDetails WHERE DicomID = " & DicomId & " AND nPatientID = " & gnPatientID & "   "
            objCmd.CommandText = "DELETE  DicomDetails WHERE DicomID = " & DicomId & " AND nPatientID = " & PatientID & "   "
            'end modification 
    ...
    ''20120718
    ''Update DICOM Files for selected patient
    Public Function UpdateDICOMFile(ByVal DicomId As Int64, ByVal PatientID As Long, ByVal DicomFileName As String)
        Dim objCon As New SqlConnection
        Dim objCmd As New SqlCommand
        Dim _result As Boolean = False
        Try
            ...
            objCon.Open()
            objCmd.CommandType = CommandType.Text
            objCmd.CommandText = "Update  DicomDetails set DICOMFileName = '" & DicomFileName & "'  WHERE DicomID = " & DicomId & " AND nPatientID = " & PatientID & "   "
            objCmd.Connection = objCon
            If (objCmd.ExecuteNonQuery() > 0) Then
                _result = True
            End If
    ...
    Public Function CheckValidFileName(ByVal fileName As String) As Boolean
        Dim invalidCharString As String = ":" + "*" + "?" + "<" + ">" + "\" + "/" + "." + "|" + "'" + ";"
        Dim chars As Char() = invalidCharString.ToCharArray()
        Dim isValid As Boolean = True
        For charCount As Integer = 0 To chars.Length - 1
            Dim strCharacter As String = chars(charCount).ToString()
            Dim _result As Boolean = fileName.Contains(strCharacter)
            If _result = True Then
                MessageBox.Show("File name is not in valid format.", "gloEMR", MessageBoxButtons.OK, MessageBoxIcon.Information)
                isValid = False
                Exit For
            End If
        Next
