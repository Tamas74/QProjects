...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmVWHistory))
        ...
        Me.pnlLeftTop.Size = New System.Drawing.Size(245, 408)
        ...
        Me.trvCategory.Size = New System.Drawing.Size(240, 398)
        ...
        Me.dgHistory.Size = New System.Drawing.Size(597, 430)
...
#End Region
    ...
    Private Sub BindGrid(Optional ByVal strcolumnName As String = "", Optional ByVal strSortBy As String = "", Optional ByVal strsearchtxt As String = "")
        Try
            If key <> -1 Then
                HistoryDBLayer.FetchData(key)
                dgHistory.SetDataBinding(HistoryDBLayer.DsDataview, "")

                HistoryDBLayer.SortDataview(HistoryDBLayer.DsDataview.Table.Columns(1).ColumnName)
                ...
                Else
                    ...
                    HistoryDBLayer.SortDataview(strColumn, strSortBy)
                End If
                ''''''''''''''''''''''''''''''''
                'Comented by Shubhangi 20091202
                'HideColumn()
            End If
            ...
            If key <> -1 Then
                If dgHistory.VisibleRowCount >= 1 Then
                    If CType(dgHistory.Item(dgHistory.CurrentRowIndex, 4), Boolean) = True Then
                        If strHistory = "OB Genetic History" Or strHistory = "OB Medical History" Or strHistory = "OB Infection History" Or strHistory = "OB Initial Physical Examination" Then
                        Else
                    ...
                    ''''''''''''''''''
                    frm = New HistoryMaster(key, ID)
                    frm.Text = "Update History"
                    frm._SelectedCategoty = strHistory

                    If Convert.ToBoolean(dgHistory.Item(dgHistory.CurrentRowIndex, 4)) = True Then
                        frm.txtDescription.Enabled = False
                        frm.cmb_HistoryCategory.Enabled = False
                    Else
    ...
    Private Sub HideColumn()
        Dim ts As New clsDataGridTableStyle(HistoryDBLayer.DsDataview.Table.TableName)

        Dim dgID As New DataGridTextBoxColumn

        With dgID
            .MappingName = HistoryDBLayer.DsDataview.Table.Columns(0).ColumnName
        ...
        With dgCol1
            .MappingName = HistoryDBLayer.DsDataview.Table.Columns(1).ColumnName
        ...
        With dgCol2
            .MappingName = HistoryDBLayer.DsDataview.Table.Columns(2).ColumnName
            .HeaderText = "Comments"
            .NullText = ""
            .Width = 0.2 * dgHistory.Width
        End With
        'Sanjog - Added on 2011 march 23 to add column for Concept ID
        Dim dgCol3 As New DataGridTextBoxColumn
        With dgCol3
            .MappingName = HistoryDBLayer.DsDataview.Table.Columns(3).ColumnName
        ...
        With dgCol4
            .MappingName = HistoryDBLayer.DsDataview.Table.Columns(4).ColumnName
        ...
        With dgCol5
            .MappingName = HistoryDBLayer.DsDataview.Table.Columns(5).ColumnName
            .HeaderText = "Snomed"
            .NullText = ""
    ...
    Private Sub txtSearch_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtSearch.TextChanged
        If _blnSearch = True Then
            ...
            Catch objErr As Exception
                Me.Cursor = Cursors.Default
                MessageBox.Show(objErr.ToString, "History", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Try
        End If
    End Sub
    ...
    Private Sub FillTreeView()
        Dim dc As DataTable
        Try
            dc = HistoryDBLayer.FillControls
            Dim mynode As myTreeNode
    ...
    Private Sub frmVWHistory_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
        Try
    ...
    Private Sub dgHistory_MouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dgHistory.MouseDoubleClick
        ...
        Catch ex As Exception
            MessageBox.Show(ex.ToString, "History", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Private Sub AddCategory()
        Try
            If strHistory <> "Medical Condition" And strHistory <> "Coded History" Then
                ...
                If key <> -1 Then
                    Dim frm As HistoryMaster
                    frm = New HistoryMaster(key)
                    'frm.Text = "Add History for " & strHistory
                    frm.Text = "Add History"
                    frm._SelectedCategoty = strHistory
                    frm.ShowDialog(IIf(IsNothing(frm.Parent), Me, frm.Parent))
                    If frm._DialogResult = Windows.Forms.DialogResult.OK Then
                        BindGrid()
                    ...
                    If dgHistory.VisibleRowCount >= 1 Then
                        ...
                        Else
                            If MessageBox.Show("Are you sure you want to delete this Record?", "gloEMR", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.Yes Then
                                ...
                                ID = CType(dgHistory.Item(dgHistory.CurrentRowIndex, 0), Long)
                                sDecription = CType(dgHistory.Item(dgHistory.CurrentRowIndex, 1), String)
                                Try
                                    If CheckIsHistoryItemExistsinPortal(ID) Then
                                        Exit Sub
                                    End If
                                    HistoryDBLayer.DeleteData(ID, sDecription)
                                    ''''''Code is Added by Anil 0n 20071102
                                    Dim myDataView As DataView = CType(dgHistory.DataSource, DataView)
                                    If (IsNothing(myDataView) = False) Then
    ...
    Private Sub trvCategory_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles trvCategory.MouseDown
        ...
        oMenuItem.Text = "Add History Category"
