...
Public Class frmTaxID_View
    ...
    Private sDatabaseconnectionstring As String = ""
    ...
    Public Sub New(sconnectionString As String)
        InitializeComponent()
        sDatabaseconnectionstring = sconnectionString
    End Sub
    ...
    Public Function GetTaxIDs() As DataTable
        ...
        Try
            ...
            objCon.ConnectionString = sDatabaseconnectionstring
            ...
            Select Case e.ClickedItem.Tag
                Case "Add"
                    Try
                        ...
                        Dim oTaxIDSetup As New frmTINMaster(sDatabaseconnectionstring, 0, 0)
                        ...
                        Designgrid()
                        gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.Setup, gloAuditTrail.ActivityType.Add, "New Tax ID setup added ", 0, 0, 0, gloAuditTrail.ActivityOutCome.Success, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)
                        'oCVsetup.Dispose()

                    Catch ex As Exception
                        gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.Setup, gloAuditTrail.ActivityType.Add, "New Tax ID setup failed ", 0, 0, 0, gloAuditTrail.ActivityOutCome.Failure, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)
                        MessageBox.Show(ex.ToString, gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    End Try
                Case "Modify"
                    ModifyTaxID()
                Case "Delete"
                    If (C1TaxID.Rows.Count > 1 AndAlso C1TaxID.RowSel > 0) Then
                        ...
                        If (Not IsTinInTransaction(nTaxMasterID)) Then
                            ...
                            End If
                        Else
                            MessageBox.Show("Selected record is associated with provider(s) transaction, you can not delete this record. ", gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        End If
                ...
                Case "Refresh"
                    Me.Cursor = Cursors.WaitCursor
                    Designgrid()
                    gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.None, gloAuditTrail.ActivityType.Refresh, "Tax ID List Refreshed", 0, 0, 0, gloAuditTrail.ActivityOutCome.Success, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)

                Case "Close"
                    Me.Close()
                    gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.CloseTransaction, gloAuditTrail.ActivityType.Close, "Tax ID List Closed", 0, 0, 0, gloAuditTrail.ActivityOutCome.Success, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)
    ...
    Private Sub DeleteTaxID(ByVal Id As Int64, ByVal sTaxID As String)
        ...
        Try
            ...
            If (objCmd.ExecuteNonQuery() > 0) Then
                ...
                If (objCmd.ExecuteNonQuery() > 0) Then
                    gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.ProviderMultipleTaxIDView, gloAuditTrail.ActivityType.Delete, "Tax ID ='" & sTaxID & "' setup deleted", 0, nTaxMasterID, 0, gloAuditTrail.ActivityOutCome.Success, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)
                End If

            End If
        Catch ex As SqlException
            gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.ProviderMultipleTaxIDView, gloAuditTrail.ActivityType.Delete, "Tax ID='" & sTaxID & "' setup deleted", 0, nTaxMasterID, 0, gloAuditTrail.ActivityOutCome.Failure, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)
            MessageBox.Show(ex.ToString(), gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Error)
        Catch ex As Exception
        ...
        If C1TaxID.RowSel > 0 Then
            ...
            If IsNothing(C1TaxID.GetData(C1TaxID.RowSel, Col_IsActive)) = False Then
                ...
                If _sStatus.ToUpper() = "ACTIVE" Then
                    ...
                    tsbtn_Act_Deact_Rule.Image = My.Resources.Deactivate
                Else
                    tsbtn_Act_Deact_Rule.Text = "&Activate"
                    tsbtn_Act_Deact_Rule.Tag = "ACTIVATE"
                    tsbtn_Act_Deact_Rule.Image = My.Resources.Activate  ''Global.gloEMR.My.Resources.Activate
                End If
            End If
        End If
    End Sub
                ...
                With C1TaxID
                    ...
                    If _SelectedTaxId > 0 Then
                        ...
                        If tsbtn_Act_Deact_Rule.Tag = "DEACTIVATE" Then
                            If Not MessageBox.Show("Are you sure you want to de-activate the current record ?", gstrMessageBoxCaption, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.Yes Then
                                Exit Sub
                            End If
                        End If
                        ...
                        If tsbtn_Act_Deact_Rule.Tag = "ACTIVATE" Then
                            If Not MessageBox.Show("Are you sure you want to activate the current record ?", gstrMessageBoxCaption, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.Yes Then
                                Exit Sub
                            End If
                        End If
                        ...
                        If tsbtn_Act_Deact_Rule.Tag = "ACTIVATE" Then
                            If UpdateActiveStatus(_SelectedTaxId, _SelectedTIN, ActiveStatus.Active.GetHashCode()) > 0 Then
                                gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.Setup, gloAuditTrail.ActivityType.Modify, "'" & _SelectedTIN & "' successfully activated", 0, _SelectedTaxId, 0, gloAuditTrail.ActivityOutCome.Success, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)
                            End If

                        Else
                            If UpdateActiveStatus(_SelectedTaxId, _SelectedTIN, ActiveStatus.InActive.GetHashCode()) > 0 Then
                                gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.Setup, gloAuditTrail.ActivityType.Modify, "'" & _SelectedTIN & "' successfully De-activated", 0, _SelectedTaxId, 0, gloAuditTrail.ActivityOutCome.Success, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)
                            End If
    ...
    Private Function UpdateActiveStatus(ByVal Id As Int64, ByVal sTaxID As String, ByVal ActiveStatus As Integer) As Integer
        ...
        Try
            ...
            objCon.Open()
            objCmd.CommandType = CommandType.Text
            objCmd.CommandText = "UPDATE dbo.TINMaster SET  dbo.TINMaster.bIsActive = " & ActiveStatus & ",  dbo.TINMaster.LoginSessionID = dbo.Get_AuditSessionID(),  dbo.TINMaster.dtModifiedDate = dbo.gloGetDate() WHERE dbo.TINMaster.nTINMasterID= " & Id & ""
            objCmd.Connection = objCon
    ...
    Private Sub ModifyTaxID()
        Try
            ...
            If (C1TaxID.Rows.Count > 1) Then
                nTaxMasterID = Convert.ToInt64(C1TaxID.GetData(C1TaxID.RowSel, Col_ID))
                sTaxID = Convert.ToString(C1TaxID.GetData(C1TaxID.RowSel, Col_TIN))
                Dim oTaxIDSetup As New frmTINMaster(sDatabaseconnectionstring, nTaxMasterID, 0, True, IsTinInTransaction(nTaxMasterID))
                oTaxIDSetup.ShowDialog(Me)
                Designgrid()
                gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.Setup, gloAuditTrail.ActivityType.Modify, "Tax ID ='" & sTaxID & "' Record Modified", 0, nTaxMasterID, 0, gloAuditTrail.ActivityOutCome.Success, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)

            End If
        Catch ex As Exception
            gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.ProviderMultipleTaxID, gloAuditTrail.ActivityCategory.Setup, gloAuditTrail.ActivityType.Modify, "Tax ID ='" & sTaxID & "' Record not Modified", 0, nTaxMasterID, 0, gloAuditTrail.ActivityOutCome.Failure, gloAuditTrail.SoftwareComponent.gloEMRAdmin, True)
            MessageBox.Show(ex.ToString, gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
