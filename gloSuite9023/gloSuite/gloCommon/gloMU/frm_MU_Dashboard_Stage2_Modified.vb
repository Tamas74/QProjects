...
Public Class frm_MU_Dashboard_Stage2_Modified
    ...
    Private ROW_ClinicalDecisionDrugInteractionCheck As Integer = 8 '4'2

    Private ROW_PatientElectronicAccess As Integer = 9
    Private ROW_PatientElectronicAccess_Measure1 As Integer = 10
    Private ROW_PatientElectronicAccess_Measure2 As Integer = 11

    Private ROW_ProtectElectronicHealthInformation As Integer = 12
    Private ROW_PatientSpecificEducation As Integer = 13
    Private ROW_MedicalReconcilation As Integer = 14
    Private ROW_SummaryOfCare As Integer = 15
    Private ROW_SecureElectronicMessaging As Integer = 16 '30

    Private ROW_PublicHealthMeasures As Integer = 17 '30
    Private ROW_ImmunizationRegistry As Integer = 18 '30
    Private ROW_SyndromicServilance As Integer = 19
    Private ROW_SpecializedRegistryReporting As Integer = 20
    ...
    Public mycaller As frm_MU_ViewDashboard_Stage2_Modified
    ...
    Private Sub FillData()
        ...
        Try
            ...
            'eRx
            dt = GetdataWithParam("MU_ePrescribedReport_Stage2", dtProviders, StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_PatientElectronicAccess_Measure1_Stage2", dtProviders, StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_PatientElectronicAccess_Measure2_Stage2", dtProviders, StartDate, Enddate, False)
            ...
            dt = GetdataWithParam("MU_PatientSpecificEducation_Stage2", dtProviders, StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_Medication_Reconciliation_Stage2", dtProviders, StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            'Summary of care
            dt = GetdataWithParam("MU_SummaryofCare_Measure2_Stage2", dtProviders, StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            ''procedure renamed and also condition for numerator commented bCommMessageType=1 
            dt = GetdataWithParam("MU_SecureElectronicMessaging_Stage2", dtProviders, StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            'Immunization 
            dt = GetdataWithParam("MU_SubmitImmunization_Stage2", dtProviders, StartDate, Enddate)
            ...
            dt = GetdataWithParam("MU_SubmitsurveillanceData_Stage2", dtProviders, StartDate, Enddate)
            ...
            dt = Getdata("MU_SpecializedRegistryReporting_Stage2")
    ...
    Private Sub FillMeasures()
        ...
        Try
            ...
            'C1QualityMeasures.Rows(ROW_PatientElectronicAccess).AllowEditing = False
            ...
            'Set the checkboxes
            For intCoreMeasures As Integer = ROW_CPOEMainMeasure To ROW_SpecializedRegistryReporting   'ROW_ProtectElectronicHealthInformation
    ...
    Private Sub FillFromDT(ByVal rptid As Int64)
        ...
        Try
            ...
            oDB.Retrive("MU_Select_MainMeasure_MST_Stage2_Modified", oParameters, dt)
            oParameters.Clear()
            ...
            If dt.Rows.Count > 0 Then
                ...
                oDB.Retrive("MU_Select_MainMeasure_DTL_Stage2_Modified", oParameters, dt)
                oParameters.Clear()
                ...
                If dt.Rows.Count > 0 Then
                    For i = 1 To C1QualityMeasures.Rows.Count - 1
                        ...
                        ElseIf i = ROW_ClinicalDecisionSupport Then
                            ...
                            dtrwno = dtrwno + 1
                            Continue For
                        ElseIf i = ROW_SpecializedRegistryReporting Then
    ...
    Private Sub C1QualityMeasures_AfterEdit(ByVal sender As Object, ByVal e As C1.Win.C1FlexGrid.RowColEventArgs) Handles C1QualityMeasures.AfterEdit
        ...
        Try
            ...
            If blnIsLoading = False Then
                ...
                If e.Col = COL_calNumerator And (e.Row = ROW_ClinicalDecisionSupport Or e.Row = ROW_ClinicalDecisionDrugInteractionCheck Or e.Row = ROW_ImmunizationRegistry Or e.Row = ROW_SyndromicServilance Or e.Row = ROW_SpecializedRegistryReporting) Then
                    'If (String.IsNullOrEmpty(Convert.ToString(C1QualityMeasures.GetData(e.Row, e.Col)))) Then
    ...
    Private Sub PrintReport()
        ...
        Try
            ...
            If SaveReport() Then
                ...
                clsPrntRpt.PrintReport("rptMU_stage2_Modified", "ReportID,User", nReportId & "," & _LoginName, gblnDefaultPrinter, "")
    ...
    Private Function SaveReport() As Boolean
        ...
        oParameter = Nothing
        '''''''''''''''''''''''''
        oDB.Execute("MU_InUp_MainMeasures_MST_Stage2_Modified", oParameters, oParameters(0).Value)
        'nReportId = oDB.ExecuteScalar("MU_InUp_MainMeasures_MST")
        nReportId = oParameters(0).Value
        'oParameters = Nothing
        oParameters.Clear()
        '''''''''''''''''''''''''''''''''Insert Or Update In Details Table''''''''''''''''''''''''
        For i = ROW_CPOEMedication To ROW_SpecializedRegistryReporting
            ...
            If i = ROW_ClinicalDecisionSupportRule Or i = ROW_PatientElectronicAccess Or i = ROW_PublicHealthMeasures Then 'Or i = ROW_SyndromicServilance
            ...
            If i = ROW_ProtectElectronicHealthInformation Then
                ...
                oDB.Execute("MU_InUp_MainMeasures_DTL_Stage2_Modified", oParameters)
                oParameters.Clear()
        ...
        Try
            If blnIsLoading = False Then
                ...
                If e.Col = COL_rptDenominator Or e.Col = COL_rptNumerator Or e.Col = COL_Copybtn Then
                    ...
                    If e.Row = ROW_ClinicalDecisionDrugInteractionCheck Or e.Row = ROW_ClinicalDecisionSupport Or e.Row = ROW_ProtectElectronicHealthInformation Or e.Row = ROW_ImmunizationRegistry Or e.Row = ROW_SyndromicServilance Then
                        e.Cancel = True
                    End If

                End If

                If e.Col = COL_Check And (e.Row <> ROW_CPOERadiology AndAlso e.Row <> ROW_CPOELabs AndAlso e.Row <> ROW_PublicHealthMeasures AndAlso e.Row <> ROW_PublicHealthMeasures AndAlso e.Row <> ROW_ImmunizationRegistry AndAlso e.Row <> ROW_SyndromicServilance AndAlso e.Row <> ROW_SpecializedRegistryReporting) Then
                    e.Cancel = True
                End If
    ...
    Private Sub C1QualityMeasures_DoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles C1QualityMeasures.DoubleClick
        Try
            ...
            If htInfo.Type = C1.Win.C1FlexGrid.HitTestTypeEnum.Cell Then
                ...
                If (C1QualityMeasures.Row = ROW_ClinicalDecisionDrugInteractionCheck Or C1QualityMeasures.Row = ROW_ClinicalDecisionSupport Or C1QualityMeasures.Row = ROW_ProtectElectronicHealthInformation Or C1QualityMeasures.Row = ROW_ImmunizationRegistry Or C1QualityMeasures.Row = ROW_SyndromicServilance Or C1QualityMeasures.Row = ROW_SpecializedRegistryReporting) Then
                ...
                With ofrm
                    ...
                    If ofrm.StoreProcedureName = "MU_PatientElectronicAccess_Measure2_Stage2" Then
