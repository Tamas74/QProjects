...
Public Class frm_MU_Dashboard_Stage1
    ...
    Private ROW_ProtectEleHealthInfo As Integer = 16
    Private ROW_MenuSet As Integer = 17
    Private ROW_DrugFormularyCheck As Integer = 18
    Private ROW_ClinicallabTest As Integer = 19
    Private ROW_GenerateListofPatient As Integer = 20
    Private ROW_Reminderstopatient As Integer = 21
    ...
    Public mycaller As frm_MU_ViewDashboard_Stage1
    ...
    Private Sub SetGridStyle()
        Try
            ...
            'Aniket: Change number of rows here
    ...
    Private Sub FillData()
        ...
        Try
            ...
            If GetAdminsetting("CPOE MU STAGE 1 2013") = "0" Then
                dt = GetdataWithParam("MU_CPOE_MedicationOrders_Stage1", ProviderID.ToString(), StartDate, Enddate)
            Else
            ...
            dt = GetdataWithParam("MU_ProblemList_Stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
            ...
            dt = GetdataWithParam("MU_ePrescribedReport_Stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_ActiveMedication_Stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_MedicationAllergy_stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            If GetAdminsetting("VITAL MU STAGE 1 2013") = "0" Then
                dt = GetdataWithParam("MU_PatientVitalReport_Stage1", ProviderID.ToString(), StartDate, Enddate)
            Else
            ...
            dt = Getdata("MU_DMRulesSetup_Stage1")
            If dt.Rows.Count > 0 Then
            ...
            dt = GetdataWithParam("MU_ClinicalSummaryUsage_Stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_PatientReminderUsage_Stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_PatientSpecificEducation_Stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_Medication_Reconciliation_Stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
            ...
            dt = GetdataWithParam("MU_SummaryCareTransitions_Stage1", ProviderID.ToString(), StartDate, Enddate)
            If dt.Rows.Count > 0 Then
                Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
    ...
    Private Sub HideRowsAsPerSettings()
        ...
        '    C1QualityMeasures.Rows(ROW_PatientElectronicAccess).Visible = False
        'End If
    ...
    Private Sub FillMeasures()
        Try
            ...
            'Set the checkboxes
            For intCoreMeasures As Integer = ROW_CPOEMedication To ROW_ProtectEleHealthInfo

                If intCoreMeasures <> ROW_PatientElectronicAccess_Measure1 And intCoreMeasures <> ROW_PatientElectronicAccess_Measure2 Then
    ...
    Private Sub FillFromDT(ByVal rptid As Int64)
        ...
        Try
            ...
            oDB.Retrive("MU_Select_MainMeasure_MST_Stage1", oParameters, dt)
            oParameters.Clear()
            ...
            If dt.Rows.Count > 0 Then
                ...
                oDB.Retrive("MU_Select_MainMeasure_DTL_Stage1", oParameters, dt)
                oParameters.Clear()
                Dim dtrwno As Integer = 0
                If dt.Rows.Count > 0 Then
                    For i = 1 To C1QualityMeasures.Rows.Count - 1
                        If i = ROW_MEnuSet Or i = ROW_PatientElectronicAccess Then
                            Continue For
                        End If


                        If i >= ROW_DrugFormularyCheck Then
                ...
                If e.Col <> 0 And e.Col <> COL_Copybtn Then
                    ...
                    Else
                        ...
                        If Convert.ToString(C1QualityMeasures.GetData(C1QualityMeasures.Row, COL_rptDenominator)) <> "" AndAlso Convert.ToString(C1QualityMeasures.GetData(C1QualityMeasures.Row, COL_rptNumerator)) <> "" Then
                            ...
                            If Convert.ToString(C1QualityMeasures.GetData(C1QualityMeasures.Row, COL_rptDenominator)) <> "" AndAlso Convert.ToString(C1QualityMeasures.GetData(C1QualityMeasures.Row, COL_rptNumerator)) <> "" Then
                                If _Deno >= 0 AndAlso _Num >= 0 Then
                                    If e.Row = ROW_Vitals Or e.Row = ROW_SmokingStatus Or e.Row = ROW_PatientElectronicAccess Or e.Row = ROW_ClinicalSummaryforPatient Or e.Row = ROW_ClinicallabTest Or e.Row = ROW_Reminderstopatient Or e.Row = ROW_MedicalReconcilation Or e.Row = ROW_SummaryCareTransition Then
                                        If _Deno = 0 Then
        ...
        Try
            ...
            If rowno = 0 Then
                For k = ROW_CPOEMedication To ROW_ProtectEleHealthInfo
                    If k = ROW_DICheck Or k = ROW_CQM Or k = ROW_DecisionSupportRule Or k = ROW_ProtectEleHealthInfo Then
                        Continue For
                    End If
    ...
    Private Sub PrintReport()
        ...
        Try
            ...
            If SaveReport() Then
                ...
                clsPrntRpt.PrintReport("rptMU_stage1", "ReportID,User", nReportId & "," & _LoginName, gblnDefaultPrinter, "")
    ...
    Private Function SaveReport() As Boolean
        Try
            ...
            oParameter = Nothing
            '''''''''''''''''''''''''
            oDB.Execute("MU_InUp_MainMeasures_MST_Stage1", oParameters, oParameters(0).Value)
            ...
            For i = ROW_CPOEMedication To ROW_SyndromicServilance
                ...
                If i = ROW_CQM Or i = ROW_ProtectEleHealthInfo Then
                    oParameter = New gloDatabaseLayer.DBParameter()
                    oParameter.ParameterName = "@nID"
                    oParameter.ParameterDirection = ParameterDirection.Input
                    oParameter.DataType = SqlDbType.BigInt
                    ...
                    oDB.Execute("MU_InUp_MainMeasures_DTL_Stage1", oParameters)
                    oParameters.Clear()
            ...
            Else
                ...
                gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.MUReports, gloAuditTrail.ActivityCategory.Reports, gloAuditTrail.ActivityType.Add, "New MU Stage 1 2014+ dashboard created with the name '" & txtReportName.Text & "' for the Provider '" & cmb_Provider.Text & "' with the measurement period From '" & Format(dtpicStartDate.Value, "MM/dd/yyyy") & "' to '" & Format(dtpicEndDate.Value, "MM/dd/yyyy") & "'.", 0, 0, cmb_Provider.SelectedValue, gloAuditTrail.ActivityOutCome.Success)
            End If
            Return True
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        ...
        Try
            If blnIsLoading = False Then
                ...
                If e.Col = COL_rptDenominator Or e.Col = COL_rptNumerator Or e.Col = COL_Copybtn Then
                    If e.Row = ROW_DICheck Or e.Row = ROW_CQM Or e.Row = ROW_DecisionSupportRule Or e.Row = ROW_ProtectEleHealthInfo Or e.Row = ROW_MEnuSet Or e.Row = ROW_DrugFormularyCheck Or e.Row = ROW_Generatelistofpatient Or e.Row = ROW_ImmunizationRegistry Or e.Row = ROW_SyndromicServilance Then
                        e.Cancel = True
                    End If
                End If
    ...
    Private Sub C1QualityMeasures_DoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles C1QualityMeasures.DoubleClick
        Try
            ...
            If htInfo.Type = C1.Win.C1FlexGrid.HitTestTypeEnum.Cell Then
                If (C1QualityMeasures.Row = ROW_DICheck Or C1QualityMeasures.Row = ROW_CQM Or C1QualityMeasures.Row = ROW_DecisionSupportRule Or C1QualityMeasures.Row = ROW_ProtectEleHealthInfo Or C1QualityMeasures.Row = ROW_Generatelistofpatient Or C1QualityMeasures.Row = ROW_ImmunizationRegistry Or C1QualityMeasures.Row = ROW_SyndromicServilance Or C1QualityMeasures.Row = ROW_DrugFormularyCheck) Then
                    Exit Sub
                End If
        ...
        Try
            Select Case ColumnNumber
                ...
                Case ROW_CPOEMedication
                    ...
                    If GetAdminsetting("CPOE MU STAGE 1 2013") = "0" Then
                        Return "MU_CPOE_MedicationOrders_Stage1"
                    Else
                        Return "MU_CPOE_MedicationOrders_MU2013"
                    End If

                Case ROW_DICheck
                    Return "MU_DIChecks_Stage1"

                Case ROW_ProblemList
                    Return "MU_ProblemList_Stage1"

                Case ROW_EPrescription
                    Return "MU_ePrescribedReport_Stage1"

                Case ROW_ActiveMedicationList
                    Return "MU_ActiveMedication_Stage1"

                Case ROW_ActiveAllergyList
                    Return "MU_MedicationAllergy_stage1"
                ...
                Case ROW_Vitals
                    ...
                    If GetAdminsetting("VITAL MU STAGE 1 2013") = "0" Then
                        Return "MU_PatientVitalReport_Stage1"
                    Else
                ...
                Case ROW_DecisionSupportRule
                    Return "MU_DMRulesSetup_Stage1"


                Case ROW_ClinicalSummaryforPatient
                    Return "MU_ClinicalSummaryUsage_Stage1"
                ...
                Case ROW_Reminderstopatient
                    Return "MU_PatientReminderUsage_Stage1"

                Case ROW_PatientSpecificEducatiion
                    Return "MU_PatientSpecificEducation_Stage1"

                Case ROW_MedicalReconcilation
                    Return "MU_Medication_Reconciliation_Stage1"


                Case ROW_SummaryCareTransition
                    Return "MU_SummaryCareTransitions_Stage1"
