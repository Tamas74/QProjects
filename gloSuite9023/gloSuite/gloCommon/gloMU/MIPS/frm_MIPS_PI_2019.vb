...
Public Class frm_MIPS_PI_2019
    ...
    Private COL_Base As Integer = 15
    Private COL_PerformancePoints As Integer = 16
    Private COL_ExclusionClaimed As Integer = 17
    Private COL_PerformanceScore As Integer = 18
    Private ROW_ProtectElectronicHealthInformation As Integer = 20
    ...
    Private ROW_EPrescription As Integer = 1 '4
    Private Row_PDMP As Integer = 2
    Private Row_OTA As Integer = 3
    ...
    Private ROW_PatientElectronicAccess_Measure1 As Integer = 4
    ...
    Private ROW_SecureElectronicMessaging As Integer = 10 '30
    'Private ROW_PatientGeneratedHealthData As Integer = 7 '30


    'Private ROW_HealthInformationExchange As Integer = 15
    Private ROW_SummaryOfCare As Integer = 5
    'Private ROW_PatientRequeustOrAccept As Integer = 10
    'Private ROW_RecieveIncorporateHealthInfo As Integer = 6
    Private ROW_MedicalReconcilation As Integer = 6
    ...
    Private ROW_ImmunizationRegistry As Integer = 7 '30
    Private ROW_SyndromicServilance As Integer = 8
    'Private ROW_SpecializedRegistryReporting As Integer = 9
    ...
    Public mycaller As frm_View_MIPS_2019
    ...
    Private Sub SetGridStyle()
        Try
            ...
            C1QualityMeasures.SetData(0, COL_PerformancePoints, "Score")
            C1QualityMeasures.SetData(0, COL_PerformanceScore, "Max")
            C1QualityMeasures.SetData(0, COL_ExclusionClaimed, "Exclude")
            ...
            C1QualityMeasures.Cols(COL_ExclusionClaimed).ImageAlign = C1.Win.C1FlexGrid.TextAlignEnum.CenterCenter
            'C1QualityMeasures.Cols(COL_Base).StyleNew.ImageAlign = C1.Win.C1FlexGrid.TextAlignEnum.CenterCenter
            ' ''Width
            C1QualityMeasures.Cols(COL_Objective).Width = Width * 0.18
            C1QualityMeasures.Cols(COL_Check).Width = Width * 0.04
            C1QualityMeasures.Cols(COL_Measure).Width = Width * 0.37
            ...
            C1QualityMeasures.Cols(COL_calNumerator).Width = Width * 0.11
            C1QualityMeasures.Cols(COL_calDenominator).Width = Width * 0.065
            C1QualityMeasures.Cols(COL_calPercent).Width = Width * 0.05
            'C1QualityMeasures.Cols(COL_calFlag1).Width = Width * 0.02
            C1QualityMeasures.Cols(COL_calFlag2).Width = Width * 0.03
            ...
            C1QualityMeasures.Cols(COL_PerformancePoints).Width = Width * 0.04
            'C1QualityMeasures.Cols(COL_Base).Width = 50
            C1QualityMeasures.Cols(COL_PerformanceScore).Width = Width * 0.036
            C1QualityMeasures.Cols(COL_ExclusionClaimed).Width = Width * 0.044
            C1QualityMeasures.ExtendLastCol = False

            C1QualityMeasures.Cols(COL_Base).Visible = False
            ...
            C1QualityMeasures.Cols(COL_ExclusionClaimed).AllowEditing = True
            C1QualityMeasures.Cols(COL_PerformancePoints).AllowEditing = False
            C1QualityMeasures.Cols(COL_PerformanceScore).AllowEditing = True

            'Aniket: Increase Rows here
            C1QualityMeasures.Rows.Add(8) ''17
    ...
    Private Sub FillData()
        ...
        Dim IsExclusionClaimed_RecieveReferral As Boolean = False
        Dim IsExclusionClaimed_ClinicalDataExchange As Boolean = False
        Try
            ...
            If Not IsNothing(cmb_Provider.DataSource) Then
                ...
                'eRx
                dt = GetdataWithParam("MU_ePrescribing_Stage3_2019", dtProviders, StartDate, Enddate, cmb_taxid.Text.ToString())
                dtCount.Merge(dt)
                If Not IsNothing(dt) Then
                    If dt.Rows.Count > 0 Then
                        ...
                        MaxPoints = 0
                        MaxPoints = C1QualityMeasures.GetData(ROW_EPrescription, COL_PerformanceScore)
                        If Percent <> "N/A" Then
                            Performance = CalculatePerformance(Percent, MaxPoints)
                            ...
                            C1QualityMeasures.SetData(ROW_EPrescription, COL_PerformancePoints, Performance)
                        Else
                ...
                dt = GetdataWithParam("MU_PDMP_MIPS", dtProviders, StartDate, Enddate, cmb_taxid.Text.ToString())
                dtCount.Merge(dt)
                If Not IsNothing(dt) Then
                    If dt.Rows.Count > 0 Then
                        Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
                        C1QualityMeasures.SetData(Row_PDMP, COL_calNumerator, dt.Rows(0)(0).ToString())
                        C1QualityMeasures.SetData(Row_PDMP, COL_calDenominator, dt.Rows(0)(1).ToString())
                        MaxPoints = 0
                        MaxPoints = C1QualityMeasures.GetData(Row_PDMP, COL_PerformanceScore)
                        If Percent <> "N/A" Then
                            Performance = CalculatePerformance(Percent, MaxPoints)
                            _Performance = _Performance + Performance
                            'If Percent <= 50 Then
                            '    C1QualityMeasures.SetCellImage(Row_PDMP, COL_calFlag1, ImgFlag.Images(2))
                            'Else
                            '    C1QualityMeasures.SetCellImage(Row_PDMP, COL_calFlag1, Nothing)
                            'End If
                            C1QualityMeasures.SetData(Row_PDMP, COL_calPercent, FormatNumber(Percent, 2, TriState.True) & "%")
                            C1QualityMeasures.SetData(Row_PDMP, COL_PerformancePoints, Performance)
                        Else
                            C1QualityMeasures.SetData(Row_PDMP, COL_calPercent, Percent)
                            'C1QualityMeasures.SetCellImage(Row_PDMP, COL_calFlag1, Nothing)
                        End If
                    End If
                End If
                ...
                '---Verify OPOID Treatment Agreement
                dt = GetdataWithParam("MU_Opioid_MIPS", dtProviders, StartDate, Enddate, cmb_taxid.Text.ToString())
                dtCount.Merge(dt)
                If Not IsNothing(dt) Then
                    If dt.Rows.Count > 0 Then
                        Percent = CalculatePercent(dt.Rows(0)(0).ToString(), dt.Rows(0)(1).ToString())
                        C1QualityMeasures.SetData(Row_OTA, COL_calNumerator, dt.Rows(0)(0).ToString())
                        C1QualityMeasures.SetData(Row_OTA, COL_calDenominator, dt.Rows(0)(1).ToString())
                        MaxPoints = 0
                        MaxPoints = C1QualityMeasures.GetData(Row_OTA, COL_PerformanceScore)
                        If Percent <> "N/A" Then
                            Performance = CalculatePerformance(Percent, MaxPoints)
                            _Performance = _Performance + Performance
                            'If Percent <= 50 Then
                            '    C1QualityMeasures.SetCellImage(Row_OTA, COL_calFlag1, ImgFlag.Images(2))
                            'Else
                            '    C1QualityMeasures.SetCellImage(Row_OTA, COL_calFlag1, Nothing)
                            'End If
                            C1QualityMeasures.SetData(Row_OTA, COL_calPercent, FormatNumber(Percent, 2, TriState.True) & "%")
                            C1QualityMeasures.SetData(Row_OTA, COL_PerformancePoints, Performance)
                        Else
                            C1QualityMeasures.SetData(Row_OTA, COL_calPercent, Percent)
                            'C1QualityMeasures.SetCellImage(Row_OTA, COL_calFlag1, Nothing)
                        End If
                    End If
                End If

                'Dim ClaimExclusionforrecieve As Boolean = False
                'Medication Reconciliation  ( import to check recieve of ccda )
                dt = GetdataWithParam("MU_RequestAcceptPatientCareRecord_Stage3_2019", dtProviders, StartDate, Enddate, cmb_taxid.Text.ToString())  ''sp change
                dtCount.Merge(dt)
                If Not IsNothing(dt) Then
                    If dt.Rows.Count > 0 Then
                        Performance = 0
                        ...
                        MaxPoints = 0
                        MaxPoints = C1QualityMeasures.GetData(ROW_MedicalReconcilation, COL_PerformanceScore)
                        If Percent <> "N/A" Then
                            Performance = CalculatePerformance(Percent, MaxPoints)
                            _Performance = _Performance + Performance
                            'If Percent <= 50 Then
                ...
                End If
                'Summary of care
                dt = GetdataWithParam("MU_SummaryofCare_Measure2_Stage3_2019", dtProviders, StartDate, Enddate, cmb_taxid.Text.ToString())
                dtCount.Merge(dt)
                If Not IsNothing(dt) Then
                    If dt.Rows.Count > 0 Then
                        Performance = 0
                        ...
                        MaxPoints = 0
                        MaxPoints = C1QualityMeasures.GetData(ROW_SummaryOfCare, COL_PerformanceScore)
                        'If ClaimExclusionforrecieve AndAlso MaxPoints = 20 Then
                        '    MaxPoints = MaxPoints + C1QualityMeasures.GetData(ROW_MedicalReconcilation, COL_PerformanceScore)
                        '    C1QualityMeasures.SetData(ROW_SummaryOfCare, COL_PerformanceScore, MaxPoints)
                        'End If
                ...
                If Not IsNothing(dt) Then
                    If dt.Rows.Count > 0 Then
                        ...
                        MaxPoints = 0
                        MaxPoints = C1QualityMeasures.GetData(ROW_PatientElectronicAccess_Measure1, COL_PerformanceScore)
                        If Percent <> "N/A" Then
                            Performance = CalculatePerformance(Percent, MaxPoints)
                            _Performance = _Performance + Performance
                            'If Percent <= 50 Then
    ...
    Private Sub SetScoreValues(ByVal _Performance As Decimal, Optional ByVal IsMIPSAPM As Boolean = False)
        'Base score is removed from calculation
    ...
    Private Sub FillMeasures()
        ...
        Try
            ...
            C1QualityMeasures.SetData(ROW_EPrescription, COL_PerformanceScore, "10")
            'C1QualityMeasures.SetCellImage(ROW_EPrescription, COL_Base, ImgFlag.Images(3))


            C1QualityMeasures.SetData(Row_PDMP, COL_Measure, "Query Of Prescription Drug Monitoring Program(PDMP)")
            C1QualityMeasures.SetData(Row_PDMP, COL_Objective, "")
            'C1QualityMeasures.SetData(ROW_EPrescription, COL_Goal, "50%")
            C1QualityMeasures.SetData(Row_PDMP, COL_Comment, "Query of Prescription Drug monitoring Program(PDMP)")
            C1QualityMeasures.SetData(Row_PDMP, COL_PerformanceScore, "5")


            C1QualityMeasures.SetData(Row_OTA, COL_Measure, "Verify Opioid Treatment Agreement")
            C1QualityMeasures.SetData(Row_OTA, COL_Objective, "")
            'C1QualityMeasures.SetData(ROW_EPrescription, COL_Goal, "50%")
            C1QualityMeasures.SetData(Row_OTA, COL_Comment, "Verify Opioid Treatment Agreement")
            C1QualityMeasures.SetData(Row_OTA, COL_PerformanceScore, "5")



            C1QualityMeasures.SetData(ROW_PatientElectronicAccess_Measure1, COL_Objective, "Provider To Patient Exchange")
            ...
            C1QualityMeasures.SetData(ROW_PatientElectronicAccess_Measure1, COL_Measure, "Provide Patients Electronic Access To Their Health Information")
            ...
            C1QualityMeasures.SetData(ROW_PatientElectronicAccess_Measure1, COL_PerformanceScore, "40")
            ...
            C1QualityMeasures.Rows(ROW_SummaryOfCare).AllowEditing = True
            C1QualityMeasures.SetData(ROW_SummaryOfCare, COL_Objective, "Health Information Exchange")
            C1QualityMeasures.SetData(ROW_SummaryOfCare, COL_Measure, "Support Electronic Referral Loops By Sending Health Information")
            ...
            'C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_Measure, "Support Electronic Referral Loops by Receiving and incorporating Health Information")
            'C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_Goal, "10%")
            'C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_Comment, "Request/Accept Summary of Care")
            ''C1QualityMeasures.SetCellImage(ROW_PatientRequeustOrAccept, COL_Base, ImgFlag.Images(3))
            'C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_PerformanceScore, "20")



            C1QualityMeasures.Rows(ROW_MedicalReconcilation).AllowEditing = True
            C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_Measure, "Support Electronic Referral Loops By Receiving And Incorporating Health Information")
            C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_Goal, "50%")
            C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_Comment, "The EP, eligible hospital or CAH who receives a patient from another setting of care or provider of care or believes an encounter is relevant should perform reconciliation")
            C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_PerformanceScore, "20")
            ...
            C1QualityMeasures.SetData(ROW_ImmunizationRegistry, COL_Objective, "Public Health and Clinical Data Exchange")
            ...
            'Set the checkboxes
            For intCoreMeasures As Integer = ROW_EPrescription To ROW_SyndromicServilance    'ROW_ProtectElectronicHealthInformation
                ...
                If intCoreMeasures = ROW_EPrescription Or intCoreMeasures = ROW_MedicalReconcilation Or intCoreMeasures = ROW_ImmunizationRegistry Or intCoreMeasures = ROW_SyndromicServilance Then
                    C1QualityMeasures.SetCellCheck(intCoreMeasures, COL_ExclusionClaimed, C1.Win.C1FlexGrid.CheckEnum.Unchecked)
                End If
    ...
    Private Sub SetReportingDates()
        ...
        ElseIf cmb_RptYear.SelectedValue = 2018 Then
            ...
            dtpicStartDate.Enabled = False
            dtpicEndDate.Enabled = False
        ElseIf cmb_RptYear.SelectedValue = 2019 Then

            dtpicStartDate.Value = New DateTime(2019, 1, 1)
            dtpicEndDate.Value = New DateTime(2019, 12, 31)
                    ...
                    For i = 1 To C1QualityMeasures.Rows.Count - 1
                        ...
                        If i = ROW_ImmunizationRegistry Or i = ROW_SyndromicServilance Then
                            ...
                            Else
                                C1QualityMeasures.SetCellImage(i, COL_rptFlag2, Nothing)
                            End If
                            If Convert.ToBoolean(dt.Rows(dtrwno)("ExclusionClaimed")) = True Then
                                C1QualityMeasures.SetCellCheck(i, COL_ExclusionClaimed, C1.Win.C1FlexGrid.CheckEnum.Checked)
                            Else
                                C1QualityMeasures.SetCellCheck(i, COL_ExclusionClaimed, C1.Win.C1FlexGrid.CheckEnum.Unchecked)
                            End If


                            'ElseIf i = ROW_ProtectElectronicHealthInformation Then
                            '    C1QualityMeasures.SetData(i, COL_calNumerator, dt.Rows(dtrwno)("nGoal"))
                        ElseIf (i = ROW_SecureElectronicMessaging) Then
                        ...
                        If i = ROW_MedicalReconcilation Or i = ROW_EPrescription Then
                            If Convert.ToBoolean(dt.Rows(dtrwno)("ExclusionClaimed")) = True Then
                                C1QualityMeasures.SetCellCheck(i, COL_ExclusionClaimed, C1.Win.C1FlexGrid.CheckEnum.Checked)
                            Else
                                C1QualityMeasures.SetCellCheck(i, COL_ExclusionClaimed, C1.Win.C1FlexGrid.CheckEnum.Unchecked)
                        ...
                        'If i = ROW_EPrescription Or i = ROW_SecureElectronicMessaging Or i = ROW_SummaryOfCare Or i = ROW_MedicalReconcilation Or i = ROW_ImmunizationRegistry Or i = ROW_SyndromicServilance Then
                        'If Convert.ToString(dt.Rows(dtrwno)(5)) = "N/A" Or Convert.ToString(dt.Rows(dtrwno)(5)).Trim() = "" Then
                        '    ' C1QualityMeasures.SetData(i, COL_calPercent, dt.Rows(dtrwno)(5))
                        ...
                        End If
                        If Convert.ToString(dt.Rows(dtrwno)("MaxPoints")) <> "" Then
                            C1QualityMeasures.SetData(i, COL_PerformanceScore, dt.Rows(dtrwno)("MaxPoints"))
                        End If
    ...
    Private Sub FillYear()
        ...
        If IsNothing(dtReportingPeriod) = True Then
            ...
            dtReportingPeriod.Rows.Add()
            bytRowCount += 1
            dtReportingPeriod(bytRowCount)("ReportingPeriodID") = 2019
            dtReportingPeriod(bytRowCount)("ReportingPeriod") = "2019 Calendar Year"
    ...
    Private Sub PrintReport()
        ...
        Try
            ...
            If SaveReport() Then
                ...
                clsPrntRpt.PrintReport("rptMIPSPI2019", "ReportID,User", nReportId & "," & _LoginName, gblnDefaultPrinter, "")
    ...
    Private Function SaveReport() As Boolean
        ...
        For i = ROW_EPrescription To ROW_SyndromicServilance
            ...
            If i = ROW_ImmunizationRegistry Or i = ROW_SyndromicServilance Then
                ...
                oParameter = New gloDatabaseLayer.DBParameter()
                oParameter.ParameterName = "@IsExclsuionClaimed"
                oParameter.ParameterDirection = ParameterDirection.Input
                oParameter.DataType = SqlDbType.Bit
                If C1QualityMeasures.GetCellCheck(i, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Unchecked Then
                ...
                oParameter = New gloDatabaseLayer.DBParameter()
                oParameter.ParameterName = "@MaxPoints"
                oParameter.ParameterDirection = ParameterDirection.Input
                oParameter.DataType = SqlDbType.NVarChar
                oParameter.Value = C1QualityMeasures.GetData(i, COL_PerformanceScore)
                oParameters.Add(oParameter)
                oParameter = Nothing
        ...
        Try
            Select Case ColumnNumber
                ...
                Case ROW_MedicalReconcilation
                    Return "MU_RequestAcceptPatientCareRecord_Stage3_2019"
    ...
    Private Sub numcomboselectedChange(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            If DirectCast(sender, System.Windows.Forms.ComboBox).SelectedItem.ToString() = "Yes" Then
                ...
                If C1QualityMeasures.GetData(ROW_EPrescription, COL_calNumerator) > 0 AndAlso C1QualityMeasures.GetData(ROW_PatientElectronicAccess_Measure1, COL_calNumerator) > 0 AndAlso C1QualityMeasures.GetData(ROW_SummaryOfCare, COL_calNumerator) > 0 > 0 Then
                    lblBaseScoreData.Text = "50"
                Else
                    lblBaseScoreData.Text = "0"
                End If
    ...
    Private Sub C1QualityMeasures_CellChecked(sender As Object, e As C1.Win.C1FlexGrid.RowColEventArgs) Handles C1QualityMeasures.CellChecked
        Try
            Dim MaxPoints As Integer = 0
            Dim MaxPointsReduced As Integer = 0
            If e.Row = ROW_MedicalReconcilation Then
                MaxPoints = 0
                If C1QualityMeasures.GetCellCheck(ROW_MedicalReconcilation, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Checked Then
                    MaxPoints = C1QualityMeasures.GetData(e.Row, COL_PerformanceScore)
                    MaxPoints = MaxPoints + C1QualityMeasures.GetData(ROW_SummaryOfCare, COL_PerformanceScore)
                    C1QualityMeasures.SetData(ROW_SummaryOfCare, COL_PerformanceScore, MaxPoints)
                    C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_PerformanceScore, "0")
                    RefreshCalculation1(ROW_SummaryOfCare, ROW_MedicalReconcilation)
                Else
                    MaxPoints = C1QualityMeasures.GetData(e.Row, COL_PerformanceScore)
                    MaxPoints = MaxPoints + C1QualityMeasures.GetData(ROW_SummaryOfCare, COL_PerformanceScore)
                    MaxPointsReduced = MaxPoints - 20
                    C1QualityMeasures.SetData(ROW_SummaryOfCare, COL_PerformanceScore, MaxPointsReduced)
                    C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_PerformanceScore, "20")
                    RefreshCalculation1(ROW_SummaryOfCare, ROW_MedicalReconcilation)
                End If
            ...
            End If
            MaxPoints = 0
            If e.Row = ROW_ImmunizationRegistry Or e.Row = ROW_SyndromicServilance Then
                If C1QualityMeasures.GetCellCheck(ROW_ImmunizationRegistry, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Checked AndAlso C1QualityMeasures.GetCellCheck(ROW_SyndromicServilance, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Checked Then
                    C1QualityMeasures.SetData(ROW_PatientElectronicAccess_Measure1, COL_PerformanceScore, "50")
                    C1QualityMeasures.SetData(ROW_ImmunizationRegistry, COL_PerformanceScore, "0")
                    RefreshCalculation1(ROW_PatientElectronicAccess_Measure1, ROW_ImmunizationRegistry)
                Else
                    If (C1QualityMeasures.GetCellCheck(ROW_ImmunizationRegistry, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Unchecked AndAlso C1QualityMeasures.GetCellCheck(ROW_SyndromicServilance, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Checked) Or (C1QualityMeasures.GetCellCheck(ROW_ImmunizationRegistry, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Checked AndAlso C1QualityMeasures.GetCellCheck(ROW_SyndromicServilance, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Unchecked) Then
                        MaxPoints = C1QualityMeasures.GetData(ROW_PatientElectronicAccess_Measure1, COL_PerformanceScore)
                        If MaxPoints = 50 Then
                            MaxPointsReduced = MaxPoints - 10
                            C1QualityMeasures.SetData(ROW_PatientElectronicAccess_Measure1, COL_PerformanceScore, MaxPointsReduced)
                            C1QualityMeasures.SetData(ROW_ImmunizationRegistry, COL_PerformanceScore, "0 or 10")
                            RefreshCalculation1(ROW_PatientElectronicAccess_Measure1, ROW_ImmunizationRegistry)
                        End If
                    End If
                End If
            End If
            Dim MaxPointsSummaryofCare As Integer = 0
            Dim MaxPointsRecive As Integer = 0
            MaxPoints = 0
            If e.Row = ROW_EPrescription Then
                If C1QualityMeasures.GetCellCheck(ROW_EPrescription, COL_ExclusionClaimed) = C1.Win.C1FlexGrid.CheckEnum.Checked Then
                    MaxPoints = C1QualityMeasures.GetData(e.Row, COL_PerformanceScore)
                    MaxPointsSummaryofCare = (MaxPoints / 2 + C1QualityMeasures.GetData(ROW_SummaryOfCare, COL_PerformanceScore))
                    C1QualityMeasures.SetData(ROW_SummaryOfCare, COL_PerformanceScore, MaxPointsSummaryofCare)
                    C1QualityMeasures.SetData(ROW_EPrescription, COL_PerformanceScore, "0")
                    MaxPointsRecive = (MaxPoints / 2 + C1QualityMeasures.GetData(ROW_MedicalReconcilation, COL_PerformanceScore))
                    C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_PerformanceScore, MaxPointsRecive)
                    RefreshCaluclationforHealthInformationExchange(ROW_SummaryOfCare, ROW_MedicalReconcilation, ROW_EPrescription)
                Else
                    MaxPoints = 0
                    C1QualityMeasures.SetData(ROW_EPrescription, COL_PerformanceScore, "10")
                    MaxPoints = C1QualityMeasures.GetData(ROW_EPrescription, COL_PerformanceScore)
                    MaxPoints = MaxPoints / 2
                    MaxPointsSummaryofCare = C1QualityMeasures.GetData(ROW_SummaryOfCare, COL_PerformanceScore) - MaxPoints
                    MaxPointsRecive = C1QualityMeasures.GetData(ROW_MedicalReconcilation, COL_PerformanceScore) - MaxPoints
                    C1QualityMeasures.SetData(ROW_SummaryOfCare, COL_PerformanceScore, MaxPointsSummaryofCare)
                    C1QualityMeasures.SetData(ROW_MedicalReconcilation, COL_PerformanceScore, MaxPointsRecive)
                    RefreshCaluclationforHealthInformationExchange(ROW_SummaryOfCare, ROW_MedicalReconcilation, ROW_EPrescription)
                End If
    ...
    End Sub
    Private Sub RefreshCalculation1(ByVal rowPointsgained As Integer, rowpointsreduced As Integer)
        Try
            Dim Performance As Decimal = 0
            Dim _PerfromanceTotal As Decimal = 0
            Dim TotalPerformanceScore As Decimal = 0
            Dim prevoiusPerfromanceScore As Decimal = 0
            Dim _previousperformanceScoreTotal As Decimal = 0
            Dim percent As String = C1QualityMeasures.GetData(rowPointsgained, COL_calPercent)
            percent = percent.Replace("%", "")
            Dim performanceScoreWeight As Integer = C1QualityMeasures.GetData(rowPointsgained, COL_PerformanceScore)
            Performance = CalculatePerformance(percent, performanceScoreWeight)
            C1QualityMeasures.SetData(rowPointsgained, COL_PerformancePoints, Performance)
            'prevoiusPerfromanceScore = C1QualityMeasures.GetData(rowpointsreduced, COL_PerformancePoints)

            If rowpointsreduced = ROW_ImmunizationRegistry Then
                Dim strnumvalue As String = Convert.ToString(C1QualityMeasures.GetData(ROW_ImmunizationRegistry, COL_calNumerator))
                Dim strdenomvalue As String = Convert.ToString(C1QualityMeasures.GetData(ROW_ImmunizationRegistry, COL_calDenominator))
                If (strnumvalue.Contains("Active") AndAlso strdenomvalue.Trim().Length > 2) Then
                    C1QualityMeasures.SetData(ROW_ImmunizationRegistry, COL_PerformancePoints, "10")
                ...
                Else
                    ...
                    Performance = 0
                End If
                If C1QualityMeasures.GetData(rowpointsreduced, COL_PerformanceScore) = "0" Then
                    C1QualityMeasures.SetData(ROW_ImmunizationRegistry, COL_PerformancePoints, "0")
                End If

            Else
                percent = C1QualityMeasures.GetData(rowpointsreduced, COL_calPercent)
                percent = percent.Replace("%", "")
                performanceScoreWeight = C1QualityMeasures.GetData(rowpointsreduced, COL_PerformanceScore)
                Performance = CalculatePerformance(percent, performanceScoreWeight)
                C1QualityMeasures.SetData(rowpointsreduced, COL_PerformancePoints, Performance)
            End If


           

            For index = ROW_EPrescription To ROW_ImmunizationRegistry
                _PerfromanceTotal = _PerfromanceTotal + C1QualityMeasures.GetData(index, COL_PerformancePoints)
            Next


            lblPerformanceScoreData.Text = _PerfromanceTotal
            SetScoreValues(_PerfromanceTotal, chkMIPSAPM.Checked)
    ...
    End Sub
    Private Sub RefreshCaluclationforHealthInformationExchange(ByVal rowPointsgained As Integer, ByVal rowPointsgained1 As Integer, ByVal rowpointsreduced As Integer)
        Dim Performance As Decimal = 0
        Dim _PerfromanceTotal As Decimal = 0
        Dim TotalPerformanceScore As Decimal = 0
        Dim prevoiusPerfromanceScore As Decimal = 0
        Dim _previousperformanceScoreTotal As Decimal = 0
        '--Gain Points for the two measures
        Dim percent As String = C1QualityMeasures.GetData(rowPointsgained, COL_calPercent)
        percent = percent.Replace("%", "")
        Dim performanceScoreWeight As Integer = C1QualityMeasures.GetData(rowPointsgained, COL_PerformanceScore)
        Performance = CalculatePerformance(percent, performanceScoreWeight)
        ...
        percent = C1QualityMeasures.GetData(rowPointsgained1, COL_calPercent)
        percent = percent.Replace("%", "")
        performanceScoreWeight = C1QualityMeasures.GetData(rowPointsgained1, COL_PerformanceScore)
        Performance = CalculatePerformance(percent, performanceScoreWeight)
        C1QualityMeasures.SetData(rowPointsgained1, COL_PerformancePoints, Performance)

        '--Reduce Points fro the reduced measure
        percent = C1QualityMeasures.GetData(rowpointsreduced, COL_calPercent)
        percent = percent.Replace("%", "")
        performanceScoreWeight = C1QualityMeasures.GetData(rowpointsreduced, COL_PerformanceScore)
        Performance = CalculatePerformance(percent, performanceScoreWeight)
