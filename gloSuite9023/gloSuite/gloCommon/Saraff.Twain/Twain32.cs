...
namespace Saraff.Twain
{
    ...
    public sealed class Twain32 : Component
    {
        ...
        /// Initializes a new instance of the <see cref="Twain32"/> class.
        /// </summary>
        public Twain32(string twainVersion)
        {
            this._srcds = new TwIdentity();
            this._srcds.Id = 0;
            this._filter = new _MessageFilter(this);
            ...
            if (twainVersion == "One")
            {
                this.IsTwain2Enable = false;
            }
            else
            {
                if (twainVersion == "Two")
                {
                    this.IsTwain2Enable = true;
                }
            }
            switch (Environment.OSVersion.Platform)
            {
        ...
        /// </summary>
        /// <param name="container">The container.</param>
        public Twain32(IContainer container, string twainVersion)
            : this()
        {
            container.Add(this);
        }
        ...
        /// 
        public void Acquire_WithoutClose()
        {
            //if (this.OpenDSM())
            //{
        ...
        public static string TwainPathForRemoteScan(bool IsTwain2Enable)
        {
            string sDLLPath = gloGlobal.clsMISC.GetEnvironmentSysFolder();
            //SHGetSpecialFolderPath(IntPtr.Zero, path, 0x0029, false);
            string _twainDsm = null;

            try
            {
                if (IsTwain2Enable)
                {
                   // _twainDsm = Path.ChangeExtension(Path.Combine(sDLLPath, "TWAINDSM"), ".dll");
                    if (!File.Exists(Path.ChangeExtension(Path.Combine(sDLLPath, "TWAINDSM"), ".dll")))
                    {
                        _twainDsm = Path.ChangeExtension(Path.Combine(Environment.SystemDirectory, "TWAINDSM"), ".dll");
                    }
                    else if (File.Exists(Path.ChangeExtension(Path.Combine(sDLLPath, "TWAINDSM"), ".dll")))
                    {
                        _twainDsm = Path.ChangeExtension(Path.Combine(sDLLPath, "TWAINDSM"), ".dll");
                    }
                }
                ...
                {
                    //_twainDsm = Path.ChangeExtension(Path.Combine(sDLLPath, "..\\twain_32"), ".dll");
                    if (!File.Exists(Path.ChangeExtension(Path.Combine(sDLLPath, "..\\twain_32"), ".dll")))
                    {
                        _twainDsm = Path.ChangeExtension(Path.Combine(Environment.SystemDirectory, "..\\twain_32"), ".dll");
                    }
                    else if (File.Exists(Path.ChangeExtension(Path.Combine(sDLLPath, "..\\twain_32"), ".dll")))
                    {
                        _twainDsm = Path.ChangeExtension(Path.Combine(sDLLPath, "..\\twain_32"), ".dll");
                    }
                }
            ...
            if ((this._TwainState & TwainStateFlag.DSMOpen) == 0)
            {
                ...
                switch (Environment.OSVersion.Platform)
                {
                    ...
                    default:
                        //string _twainDsm=Path.ChangeExtension(Path.Combine(Environment.SystemDirectory,"TWAINDSM"),".dll");
                        //this._hTwainDll=_Platform.Load(File.Exists(_twainDsm)&&this.IsTwain2Enable?_twainDsm:Path.ChangeExtension(Path.Combine(Environment.SystemDirectory,"..\\twain_32"),".dll"));
                        string _twainDsm = TwainPathForRemoteScan(this.IsTwain2Enable);

                        if (!string.IsNullOrEmpty(_twainDsm))
                        {
                            try
                            {
                                this._hTwainDll = _Platform.Load(_twainDsm);
                                if (this.Parent != null)
                                {
                                    this._hwnd = this.Parent.Handle;
                                }
                            ...
                            {
                                throw new TwainException(ex.ToString());
                            }
                            catch 
                            {
                                throw;
                        ...
                        else { throw new TwainException("Twain DLLs not installed!!!"); }
                        break;
                }
                ...
                {
                    throw new TwainException("Unable to load twain dll.");
                }
            }
        ...
        public TwQC IsCapSupported(TwCap capability)
        {
            if ((this._TwainState & TwainStateFlag.DSOpen) != 0)
            {
                ...
                try
                {
                    //logdata.Log("before _dsmEntry.DsInvoke");
                    TwRC _rc = this._dsmEntry.DsInvoke(this._AppId, this._srcds, TwDG.Control, TwDAT.Capability, TwMSG.QuerySupport, ref _cap);
                    //logdata.Log("after _dsmEntry.DsInvoke");
                    if (_rc == TwRC.Success)
                    {
                        //logdata.Log("after _dsmEntry.DsInvoke TwRC.Success");
                        return (TwQC)((TwOneValue)_cap.GetValue()).Item;
                    }
                    //logdata.Log("after _dsmEntry.DsInvoke return 0");
                    return 0;
                }
                finally
                {
        ...
        private void _NativeTransferPictures()
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    if (this._OnEndXfer(new EndXferEventArgs(_img, (Int16)_pxfr.Count)))
                    {
                        _img.DisposeImage();
                        return;
                    }
        ...
        private TwRC _TwCallbackProc(TwIdentity srcId, TwIdentity appId, TwDG dg, TwDAT dat, TwMSG msg, IntPtr data)
        {
            try
            {
                ...
                {
                    this._OnAcquireFailure(new AcquireErrorEventArgs(new TwainException("Failure", new Exception())));
                    return TwRC.Failure;
                }
                ...
                this._TwCallbackProcCore(msg, isCloseReq =>
                {
                    ...
                    {
                        this._DisableDataSource();
                        this._OnAcquireClosure(new AcquireErrorEventArgs(new TwainException("Closure", new Exception())));
        ...
        public bool bBeforeAquireCompletion = false;
        ...
        private void _OnAcquireFailure(AcquireErrorEventArgs e)
        {
            if (this.AcquireFailure != null)
            {
                this.AcquireFailure(this, e);
            }
        }

        private void _OnAcquireClosure(AcquireErrorEventArgs e)
        {
            if (this.AcquireClosure != null)
            {
                this.AcquireClosure(this, e);
            }
        }

        private void _OnAcquireException(AcquireErrorEventArgs e)
        {
            if (this.AcquireException != null)
            {
                this.AcquireException(this, e);
            }
        }
        ...
        [Category("Action")]
        [Description("Возникает в момент получения ошибки в процессе сканирования. Occurs when error received during acquire.")]
        public event EventHandler<AcquireErrorEventArgs> AcquireFailure;
        ...
        [Category("Action")]
        [Description("Возникает в момент получения ошибки в процессе сканирования. Occurs when error received during acquire.")]
        public event EventHandler<AcquireErrorEventArgs> AcquireClosure;
        ...
        [Category("Action")]
        [Description("Возникает в момент получения ошибки в процессе сканирования. Occurs when error received during acquire.")]
        public event EventHandler<AcquireErrorEventArgs> AcquireException;
        ...
        public sealed class EndXferEventArgs : SerializableCancelEventArgs
        {
            ...
            internal EndXferEventArgs(object image, Int16 iCnt)
            {
                this._image = image as _Image;
                this._iCnt = iCnt;
            }
            ...
            public TwRC DsInvoke<T>(TwIdentity origin, TwIdentity dest, TwDG dg, TwDAT dat, TwMSG msg, ref T data) where T : class
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if ((_rc == TwRC.Success || _rc == TwRC.DSEvent || _rc == TwRC.XferDone) && _data != null)
                        {
                            data = (T)Marshal.PtrToStructure(_data, typeof(T));
                        }
                        if (_data != null)
                ...
                try
                {
                    ...
                    switch (this._twain._dsmEntry.DsInvoke(this._twain._AppId, this._twain._srcds, TwDG.Control, TwDAT.Event, TwMSG.ProcessEvent, ref this._evtmsg))
                    {
                        case TwRC.DSEvent:
                            this._twain._TwCallbackProcCore(this._evtmsg.Message, isCloseReq =>
                            {
                                if (isCloseReq || this._twain.DisableAfterAcquire)
                                {
                                    ...
                                    this._twain._OnAcquireClosure(new AcquireErrorEventArgs(new TwainException(isCloseReq?"Closure":"Disabled", new Exception())));
                                }
                            });
                            break;
                        case TwRC.NotDSEvent:
                            return false;
                        case TwRC.Failure:
                            this._twain._OnAcquireFailure(new AcquireErrorEventArgs(new TwainException("Failure", new Exception())));
                            throw new TwainException(this._twain._GetTwainStatus(), TwRC.Failure);
                        default:
                            this._twain._OnAcquireException(new AcquireErrorEventArgs(new TwainException("Exception", new Exception())));
        ...
        public sealed class Enumeration
        {
            ...
            public static Enumeration FromRange(Range value)
            {
                ...
                object[] _items = new object[(int)((Convert.ToSingle(value.MaxValue) - Convert.ToSingle(value.MinValue)+Convert.ToSingle(1)) / Convert.ToSingle(value.StepSize))];
        ...
        public sealed class ImageMemXfer
        {
            ...
            internal static ImageMemXfer Create(TwImageMemXfer data)
            {
                ...
                {
                    if (data.Memory.TheMem != IntPtr.Zero)
                    { 
