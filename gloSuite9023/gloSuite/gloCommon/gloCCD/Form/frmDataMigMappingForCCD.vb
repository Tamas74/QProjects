...
Public Class frmDataMigMappingForCCD
    Dim ht As Hashtable
    ...
    Dim gstrMessageBoxCaption As String = "CCD/CCR"
    ...
    Private Sub trHL7Fields_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles trHL7Fields.DoubleClick
        ...
        Try
            ...
            Else
                ...
                _strSQL = "SELECT COUNT(sNodePath) FROM dbo.CCD_MultipleComplexNode WHERE sNodePath='" & mySelectedNode.FullPath & "'"
                ...
                If (AllowNode > 0) Then
                    ...
                    RefNodePath = myTNODE.RefFieldPath
                    If doc.DocumentElement.Prefix Is Nothing OrElse doc.DocumentElement.Prefix.Trim() = "" Then
                        _documentprefix = "CR"
                        nsmgr.AddNamespace(_documentprefix, doc.DocumentElement.NamespaceURI)
                        RefNodePath = GetPrefixedXPath(RefNodePath, _documentprefix)
                    Else
                        _documentprefix = doc.DocumentElement.Prefix
                        nsmgr.AddNamespace(_documentprefix, doc.DocumentElement.NamespaceURI)
                        RefNodePath = GetPrefixedXPath(RefNodePath, "")
                    End If


                    Dim _NodeList As XmlNodeList = doc.SelectNodes(RefNodePath, nsmgr)

                    For y As Int32 = 0 To _NodeList.Count - 1
                        If (Not _NodeList(y).Attributes(myTNODE.RefFieldNode) Is Nothing) Then
                            myTNODE.RefFieldValue = _NodeList(y).Attributes(myTNODE.RefFieldNode).Value.ToString()
                            Exit For
                        ElseIf (_NodeList(y).Name = myTNODE.RefFieldNode) Then
                            myTNODE.RefFieldValue = _NodeList(y).InnerText
                            Exit For
                        End If
                    Next
                    _NodeList = Nothing
                ...
                If (mySelectedNode.TableName = "Attribute") Then
                    myTNODE.FieldName = mySelectedNode.Parent.FullPath
                Else
                    myTNODE.FieldName = mySelectedNode.FullPath
                End If
    ...
    Private Function FillReferencePath() As DataTable
        ...
        Try
            cnn.ConnectionString = gloLibCCDGeneral.Connectionstring
            Dim _strSQL As String = "SELECT isnull(sNodePath,'') FROM dbo.CCD_MultipleComplexNode "
    ...
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveToolStripButton1.Click
        ...
        Dim thisNodeItem0 As MyTreeNode
        Dim thisNode As MyTreeNode
        Try
            ...
            For i As Int32 = 0 To trMappedFields.GetNodeCount(False) - 1
                ...
                thisNodeItem0 = CType(trMappedFields.Nodes.Item(i).Nodes.Item(0), MyTreeNode)
                thisNode = CType(trMappedFields.Nodes.Item(i), MyTreeNode)
            ...
            If (ogloCCR_Interface.InsertUserCCDMappingField(arry)) Then
            ...
            If Not IsNothing(myHl7DragNode) Then
                ...
                If dropNode.TreeView Is trHL7Fields Then
                    ...
                    For i As Int32 = 0 To trMappedFields.GetNodeCount(False) - 1
                        ...
                        If childnode.Text = thisNode.Text Then
                            flag = True
                            Exit For
                        Else
                            flag = False
                    ...
                    If flag = False Then
                        ...
                        trMappedFields.ExpandAll()
                    Else
                        If (IsNothing(childnode) = False) Then
                            childnode = Nothing
                        End If
    ...
    Private Sub PopulateHL7DataInbound()
        ...
        Try
            ...
            dt = ogloCCR_Interface.DefaultCCDMappingRestore(sCCRVer, "")
