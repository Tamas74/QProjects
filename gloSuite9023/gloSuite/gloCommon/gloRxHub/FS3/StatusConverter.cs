...
namespace gloRxHub.FS3
{
    public class StatusConverter : IDisposable
    {
        ...
        {
            Bitmap displayIcon = global::gloRxHub.Properties.Resources.U;
            Int32 fs = -1;

            if (Int32.TryParse(StatusCode, out fs) && fs >= 0)
            {
                if (fs == 0)
                { displayIcon = global::gloRxHub.Properties.Resources.Not_Reimbursable; }
                else if (fs == 1)
                { displayIcon = global::gloRxHub.Properties.Resources.Off_Formulary; }
                else if (fs >= 2 && fs <= 99)
                { displayIcon = global::gloRxHub.Properties.Resources.On_Formulary; }
            }
            return displayIcon;
        }
