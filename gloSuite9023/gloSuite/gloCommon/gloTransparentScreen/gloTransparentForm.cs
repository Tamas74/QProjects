...
{
    public class gloTransperentForm : System.Windows.Forms.Form
    {
        public gloTransperentForm()
        {
            if (!this.DesignMode)
            {
                m_layeredWnd = new gloLayeredWindow();
            }
            ...
            gloBackGroundColor = Color.Fuchsia;
            gloTransparancyKey = Color.Fuchsia;
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                if (m_hiddenControls != null)
                {
                    m_hiddenControls.Clear();
                }
                if (m_controlDict != null)
                {
                    m_controlDict.Clear();
                }
                   m_customLayeredWindowProc = null;
                   //m_layeredWindowProc = default(IntPtr);
                // Dispose stuff here
            }
        ...
        public int keepOpacity = 255 * 3;
        public void SetOpacity(int Opacity)
        {
            SetOpacity((double)((double)(keepOpacity + Opacity) / (double)(keepOpacity + 255)));
        }
        ...
        #endregion
        public Color gloBackGroundColor
        {
            get;
            set;
        }
        public Color gloTransparancyKey
        {
            get;
            set;
        }
        protected override void OnLoad(EventArgs e)
        {
            ...
            this.BackColor = gloBackGroundColor;
            this.TransparencyKey = gloTransparancyKey;
          ...
          //  SetTopLevel(true);
            ...
            if (!this.DesignMode)
            {
                ...
                if (m_layeredWnd != null)
                {
                    // m_layeredWnd.Text = "gloTransparentForm";
                    m_initialised = true;
        ...
        private void updateLayeredBackground(int width, int height, Point pos, bool Render)
        {
            ...
            if (this.DesignMode || m_background == null || !m_initialised)
                return;
            if ((this.Left != m_layeredWnd.Left) || (this.Top != m_layeredWnd.Top))
            {
                this.Left = m_layeredWnd.Left;
                this.Top = m_layeredWnd.Top;
            }
            switch (m_sizeMode)
            {
                ...
                if (Render)
                {
                    ...
                    foreach (KeyValuePair<Control, bool> kvp in m_controlDict)
                    {
                        ...
                        if (drawBack && ctrl.BackColor == Color.Transparent)
                        {
                            ...
                            {
                                PaintLabel(ctrl, ref g, new Point(offX+ctrl.Location.X, offY+ctrl.Location.Y));
                                //if (ctrl is gloTransparentScreen.gloLabel)
                                //{
                                //    gloTransparentScreen.gloLabel thisCtrl = ctrl as gloTransparentScreen.gloLabel;
                                //    PaintLabel(thisCtrl, ref g, new Point(thisCtrl.Location.X,thisCtrl.Location.Y));
                                //}
                                //else
                                //{
                                ...
                                //        Rectangle bmprect = new Rectangle(0, 0, ctrl.Width, ctrl.Height);
                                //        ctrl.DrawToBitmap(bmp, bmprect);

                                //        g.DrawImage(bmp, ctrl.Left, ctrl.Top, ctrl.Width, ctrl.Height);

                                //    }
                                //}
                              //  m_backgroundEx.Save(@"c:\test\Full" + gloGlobal.clsFileExtensions.GetUniqueID() + ".bmp");
        ...
        public void PaintLabel(Control lblText, ref Graphics grLabel, Point pos)
        {

            foreach (Control child in lblText.Controls)
            {
                PaintLabel(child, ref grLabel, new Point(pos.X + child.Location.X, pos.Y + child.Location.Y));
             //   m_backgroundEx.Save(@"c:\test\Child" + gloGlobal.clsFileExtensions.GetUniqueID() + ".bmp");
            }

            if (lblText.Visible)
            {
                if (lblText is gloTransparentScreen.gloLabel)
                {
                    gloTransparentScreen.gloLabel thisCtrl = lblText as gloTransparentScreen.gloLabel;
                    gloTransparentScreen.gloLabel.PaintLabel(thisCtrl, ref grLabel, pos);
                    //  m_backgroundEx.Save(@"c:\test\ChildFull" + gloGlobal.clsFileExtensions.GetUniqueID() + ".bmp");
                }
                if (lblText is gloTransparentScreen.gloLinkLabel)
                {
                    gloTransparentScreen.gloLinkLabel thisCtrl = lblText as gloTransparentScreen.gloLinkLabel;
                    gloTransparentScreen.gloLinkLabel.PaintLabel(thisCtrl, ref grLabel, pos);
                    //  m_backgroundEx.Save(@"c:\test\ChildFull" + gloGlobal.clsFileExtensions.GetUniqueID() + ".bmp");
                }
                //if (lblText is Button)
                //{
                //    Button thisCtrl = lblText as Button;
                //    if (thisCtrl.Image != null)
                //    {
                //        Rectangle controlRect = new Rectangle(pos.X, pos.Y, lblText.Size.Width, lblText.Size.Height);
                //        grLabel.DrawImage(thisCtrl.Image, controlRect);
                //    }
                //}
            }           
        }
        ...
        private gloLayeredWindow m_layeredWnd;
        private int m_offX;
        private int m_offY;
