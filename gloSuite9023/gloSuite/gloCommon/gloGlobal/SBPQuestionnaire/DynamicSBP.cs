...
{
    public partial class usrCtrlDynamicSBP : UserControl
    {
        //public usrCtrlDynamicSBP()
        //{
        //    InitializeComponent();
        ...
        public  usrCtrlDynamicSBP()
        {
           ...
           // DynamicFormData.AppConnectionString = ConnectionString;
            LoadSBPData();
        }

        //constructor called from frmSBPTransaction form
        public usrCtrlDynamicSBP(Int64 PatientId, DateTime TransactionDate,Boolean getDataFromSBPHxTable)
        {
            InitializeComponent();

            SBPTrnPatientId = PatientId;
            //SBPTrnVisitId = VisitId;
            SBPTransactionDate = TransactionDate;
            blnGetDataFromSBPHxTable = getDataFromSBPHxTable;
            // DynamicFormData.AppConnectionString = ConnectionString;
            LoadSBPData();
        }
        ...
        //used when called from frmSBPTransaction form
        Int64 SBPTrnPatientId;
        //Int64 SBPTrnVisitId;
        DateTime SBPTransactionDate;
        bool blnGetDataFromSBPHxTable = false;//if from patient details panel when we select first row that holds latest SBP data which is present in SBPTransaction table and not in SBP_Transaction_HST table hence if selected row from patient details panel is 1 then take data from SBPTransaction table else fetch data from SBP_Transaction_HST table 

        private void LoadSBPData()
        {
            gloDatabaseLayer.DBLayer oDB = null;
            gloDatabaseLayer.DBParameters oDBParameters = null;
            //WebBrowser wbBrowserCtrl = new WebBrowser();
            try
            {
...
//                                </table>";
                    ...
                    foreach (DataRowView rowView in dvSBPDataQue)
                    {
                        ...
                        if (row["sInputType"].ToString() == "ComboBox")
                        {
                            ...
                            foreach (DataRowView rowView1 in dvSBPDataAns)
                            {
                                ...
                                option = option + "<option value=" + row1["AnswerStringID"].ToString() + "_" + row1["Score"].ToString() + ">" + row1["DisplayText"].ToString() + " (" + row1["AnswerStringID"].ToString() + ")"+" </option>";
                            }
                            option = option + "<option value=ASKU >Decline to specify" + " </option>";
                            ...
                            webpage = webpage + "<tr><td>" +
                             "<label>" +
                               "<input " + txttype + " type='text'  id='" + row["sLOINCCode"].ToString() + "_text_" + row["nDomainSeqNo"].ToString() + "' onkeydown='return ( event.ctrlKey || event.altKey " +
                                      ...
                                      "<label >" +
                                      "<input type='checkbox' name='checkbox' value='checkbox' id='" + row["sLOINCCode"].ToString() + "_questionchk_" + row["nDomainSeqNo"].ToString() + "' />" +
                                      "Decline to specify</label> " +
                           "</td>"+
        ...
        public void FillFormData(long PatientId)
        {
            ...
            try
            {
                ...
                //used when form is called from frmSBPTransaction form for view mode only
                if (SBPTrnPatientId != 0)//means form is called from frmSBPTransaction form for view mode only
                {
                    if (blnGetDataFromSBPHxTable == true)
                    {
                        oDBParameters.Add("@nPatientId", SBPTrnPatientId, ParameterDirection.Input, SqlDbType.BigInt);
                        oDBParameters.Add("@TransactionDate", SBPTransactionDate, ParameterDirection.Input, SqlDbType.DateTime);
                    }
                    else
                    {
                        oDBParameters.Add("@nPatientId", SBPTrnPatientId, ParameterDirection.Input, SqlDbType.BigInt);
                ...
                else//form is called from Go menu with full control
                {
                ...
                foreach (DataRowView rowView in dv)
                {
                    ...
                    {
                        case "ComboBox":
                            if (row["RecordedAnswer"].ToString() == "G-ASKU")
                            {
                        ...
                        case "TextBox":
                            if (row["RecordedAnswer"].ToString() == "G-ASKU")
                            {
                                ...
                                wbBrowserCtrl.Document.GetElementById(row["QuestionLoincCode"].ToString() + "_questionchk_" + row["nDomainSeqno"].ToString()).SetAttribute("checked", "False");
                                wbBrowserCtrl.Document.GetElementById(row["QuestionLoincCode"].ToString() + "_questionchk_" + row["nDomainSeqno"].ToString()).Enabled = false;
                            ...
                            else
                            {
                                ...
                                {
                                    if (wbBrowserCtrl.Document.GetElementById(row["QuestionLoincCode"].ToString() + "_questionchk_" + row["nDomainSeqno"].ToString()) != null)
                                    {
                                        wbBrowserCtrl.Document.GetElementById(row["QuestionLoincCode"].ToString() + "_questionchk_" + row["nDomainSeqno"].ToString()).SetAttribute("checked", "True");
                                    }
                                    wbBrowserCtrl.Document.GetElementById(row["QuestionLoincCode"].ToString() + "_text_" + row["nDomainSeqNo"].ToString()).SetAttribute("Value", "");
                                    wbBrowserCtrl.Document.GetElementById(row["QuestionLoincCode"].ToString() + "_text_" + row["nDomainSeqNo"].ToString()).Enabled = false;
                                }
                ...
                switch (wbBrowserCtrl.Document.ActiveElement.TagName)
                {
                    case "INPUT"://Checkbox, Text box
                        ...
                        if (wbBrowserCtrl.Document.ActiveElement.Name == "checkbox")
                        {
                                ...
                                foreach (DataRowView rowView in dvSBPDataQue)
                                {
                                    ...
                                    switch (row["sInputType"].ToString())
                                    {
                                        ...
                                        case "TextBox":
                                            if (wbBrowserCtrl.Document.GetElementById(wbBrowserCtrl.Document.ActiveElement.Id).GetAttribute("checked") == "True")
                                            {
                                                wbBrowserCtrl.Document.GetElementById(row["sLOINCCode"].ToString() + "_questionchk_" + row["nDomainSeqNo"].ToString()).Enabled = false;
                                                wbBrowserCtrl.Document.GetElementById(row["sLOINCCode"].ToString() + "_questionchk_" + row["nDomainSeqNo"].ToString()).SetAttribute("checked", "True");
                                            ...
                                            {                                                                                               
                                                wbBrowserCtrl.Document.GetElementById(row["sLOINCCode"].ToString() + "_questionchk_" + row["nDomainSeqNo"].ToString()).Enabled = true;
                                                wbBrowserCtrl.Document.GetElementById(row["sLOINCCode"].ToString() + "_questionchk_" + row["nDomainSeqNo"].ToString()).SetAttribute("checked", null);
                            ...
                            else if (DomParentSeqNo[1] == "questionchk")
                            {
                                if (wbBrowserCtrl.Document.GetElementById(wbBrowserCtrl.Document.ActiveElement.Id).GetAttribute("checked") == "True")
                                {
                                    wbBrowserCtrl.Document.GetElementById(DomParentSeqNo[0].ToString() + "_text_" + DomParentSeqNo[2].ToString()).Enabled = false;
                                    wbBrowserCtrl.Document.GetElementById(DomParentSeqNo[0].ToString() + "_text_" + DomParentSeqNo[2].ToString()).SetAttribute("Value", "");

                                    //wbBrowserCtrl.Document.GetElementById(row["QuestionLoincCode"].ToString() + "_chk_" + row["nDomainSeqno"].ToString()).SetAttribute("checked", "True");
                                }
                                else
                                {
                                    wbBrowserCtrl.Document.GetElementById(DomParentSeqNo[0].ToString() + "_text_" + DomParentSeqNo[2].ToString()).Enabled = true;
                                }
                            }
                ...
                for (int i = 1; i <= maxVal; i++)
                {
                    ...
                    foreach (DataRowView rowView in dvSBPDataQue)
                    {
                        ...
                        if (row["sInputType"].ToString() == "TextBox")
                        {
                            ...
                            else if (wbBrowserCtrl.Document.GetElementById(row["sLOINCCode"].ToString() + "_questionchk_" + row["nDomainSeqNo"].ToString()).GetAttribute("checked") == "True")
                            {
                                r[1] = "ASKU";
                                Isans = true;
                            }
