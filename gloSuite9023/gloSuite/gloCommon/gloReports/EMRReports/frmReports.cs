    ...
    public partial class frmReports : Form
    {
            ...
            try
            {
                ...
                switch (_ReportNM.ToLower())
                {
                    case "alerts":
                        {
                            //form caption
                            this.Text = "Patient Alerts Report";
                            //
                            Tblbtn_More.Visible = false;
        ...
        public void Fill_Provider()
        {
            ...
            try
            {
                ...
                objCmd.CommandText = "sp_ScanProviderName";
        ...
        public DataTable FillCategDetails(string sCateg)
        {
            ...
            try
            {
                ...
                {
                    //_strSQL =" SELECT History_MST.nHistoryID AS 'ID' ,History_MST.sDescription AS 'Description', 0 AS 'IsDrug' ,'' as  'Dosage',0 as 'DDid', '' as 'NDCCode' " +         
                ...
                else
                {
                    ...
                    //" SELECT ISNULL(Drugs_MST.nDrugsID,0) AS 'ID',ISNULL(Drugs_MST.sDrugName,'') AS 'Description',1 AS 'ISDrug', " +
                    //" ISNULL(Drugs_MST.sDosage,'') as 'Dosage', ISNULL(Drugs_MST.nddid,0) as 'DDid', ISNULL(Drugs_MST.sNDCCode,'')  as 'NDCCode'  FROM DRUGS_MST WHERE " +
                    //" bIsAllergicDrug=1 ORDER BY Description ";
                    ...
                    case "dm":
                        {
                            ...
                            for (i = 0; i <= LstMedication.Items.Count - 1; i++)
                            {
                                if (i == 0)
                                {
                                    strDM = "'" + LstMedication.Items[i].ToString().Trim() + "'";
                                }
                                else
                                {
                                    strDM = strDM + " or '" + LstMedication.Items[i].ToString().Trim() + "'";
                                }
                            }
        ...
        private void FillPatientHealthPlanReport(string sHealthPn)
        {
            ...
            try
            {
                ...
                if (DsCCHITRPT.dt_PatientsDM.Rows.Count == 0)
                {
                    bNoData = true;
                }
                else
        ...
        {
            CrystalDecisions.Shared.ExportFormatType ExptFrmt = CrystalDecisions.Shared.ExportFormatType.WordForWindows;
            ...
            Sfd1.Filter = "Microsoft Word (*.doc) |*.doc |RichText Format (*.rtf) |*.rtf |Microsoft Excel (*.xls) |*.xls |Microsoft Excel [Data Only] (*.xls) |*.xls |CrystalReport Format (*.rpt) |*.rpt |Adobe Acrobat (*.pdf) |*.pdf";
            Sfd1.FilterIndex = 1;
            Sfd1.RestoreDirectory = true;
            Sfd1.CreatePrompt = true;
            Sfd1.FileName = "Report1";
        ...
        private void FillBPReportPatient(Int64 nproviderID, string sDiag)
        {
            ...
            if (cmbAge.Text != "For All")
            {
                ...
                {
                    _sqlage += "    Patient.npatientid in (select  distinct npatientid FROM Patient WHERE (SELECT (CONVERT (INT,dbo.GetPatientAgeInYears(Patient.npatientid,getdate())))) <  " + age1 + "  ) ";
                    //  + " AND  (SELECT (CONVERT (INT,dbo.GetPatientAgeInYears(Patient.npatientid,getdate())))) > 18  ";
                ...
                {
                    _sqlage += "    Patient.npatientid in (select  distinct npatientid FROM Patient WHERE (SELECT (CONVERT (INT,dbo.GetPatientAgeInYears(Patient.npatientid,getdate())))) >=  " + age1 + "  "
                        + " AND  (SELECT (CONVERT (INT,dbo.GetPatientAgeInYears(Patient.npatientid,getdate())))) <= " + age2 + "  ) ";
                ...
                else
                {
                    ...
                    _sqlage += "    Patient.npatientid in (select  distinct npatientid FROM Patient WHERE (SELECT (CONVERT (INT,dbo.GetPatientAgeInYears(Patient.npatientid,getdate())))) >=  " + age1 + "  "
             + " OR (SELECT (CONVERT (INT,dbo.GetPatientAgeInYears(Patient.npatientid,getdate())))) = " + age2 + "  ) ";
        ...
        private void FillBPRepors(string QueryForPatientIds)
        {
            ...
            try
            {
                oConnection.ConnectionString = _databaseconnectionstring;
                _sqlcommand.CommandType = CommandType.StoredProcedure;
                _sqlcommand.CommandText = "SP_GetPatientBP";
                _sqlcommand.Connection = oConnection;
                ...
                if (DsCCHITRPT.Tables["dt_PatientBP1"] != null && DsCCHITRPT.Tables["dt_PatientBP1"].Rows.Count > 0)
                {
                    ...
                    dt1 = DsCCHITRPT.Tables["dt_PatientBP1"].DefaultView.ToTable(true, "ProviderID");
