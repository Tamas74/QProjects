...
{
    public partial class frmWorkerCompFormViewer_MG2 : Form
    {
       ...
       // gloPatientStripControl.gloClaimSearchControl oPatientStripControl = null;
        ...
        public frmWorkerCompFormViewer_MG2(Int64 patientid, Int64 transactionID, Int64 transactionmasterID, string _databaseconnectionstring, Int64 formID)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                dtProperties = GetAllPropertiesInfo(objMG2);
                if (objConn != null)
                {
                ...
                oParam.Value = (int)WorkersCompFormTypes.MG2;
                ...
                if (dsWorkersCompForms != null && dsWorkersCompForms.Tables.Count > 0)
                {
                    ...
                    {
                          SetDataToMG2Object(dsWorkersCompForms.Tables["WorkersCompForms_DTL"], ref objMG2);
                        bIsNewForm = false;
                    }
                }
                ...
                if (bIsNewForm)
                {
                    ...
                    ObjWorkerComp.SetMG2PrdefinedInfo(dsInfo, ref objMG2, true, true);
                }
        ...
        public void LoadForWorkerCompViewer()
        {
            try
            {
                ...
                _pdfdoc = new PDFDoc(global::gloBilling.Properties.Resources.MG2_tab_ImageSign, global::gloBilling.Properties.Resources.MG2_tab_ImageSign.Length);
                ...
                if (objMG2.MG2_ApprovalRequestVarianceFor.ProviderSignImage != null && objMG2.MG2_ApprovalRequestVarianceFor.ProviderSignImage.Length > 0)
                {
                ...
                lblPatientName.Text = objMG2.MG2_PatientDetails.PatientsFullName;
        ...
        private void frmWorkerCompFormViewer_MG2_Load(object sender, EventArgs e)
        {
        ...
        private void SetFormFieldsDataClaimOnly(ref PDFDoc FormPdfDoc)
        {
            ...
            try
            {
                ...
                propinfo = null;
                propinfo = objMG2.MG2_CaseDetails.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_CaseDetails);
                    }
                }

                propinfo = null;
                propinfo = objMG2.MG2_AttendingDoctorInformation.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_AttendingDoctorInformation);
                    }
                }

                propinfo = null;
                propinfo = objMG2.MG2_HeaderInfo.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_HeaderInfo);
                    }
                }

                SetData(FormPdfDoc, objMG2.MG2_PatientDetails.GetType().GetProperty("InsuranceCarrierNameAndAddress"), objMG2.MG2_PatientDetails);
                SetData(FormPdfDoc, objMG2.MG2_ApprovalRequestVarianceFor.GetType().GetProperty("ProviderSignImage"), objMG2.MG2_ApprovalRequestVarianceFor);
        ...
        private void SetFormFieldsData(ref PDFDoc FormPdfDoc)
        {
            ...
            try
            {
                ...
                propinfo = objMG2.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2);
                    }
                }
                ...
                propinfo = null;
                propinfo = objMG2.MG2_PatientDetails.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_PatientDetails);
                    }
                }
                ...
                propinfo = null;
                propinfo = objMG2.MG2_ApprovalRequestVarianceFor.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_ApprovalRequestVarianceFor);
                    }
                }
                ...
                propinfo = null;
                propinfo = objMG2.MG2_IndependentMedicalExaminer.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_IndependentMedicalExaminer);
                    }
                }

                propinfo = null;
                propinfo = objMG2.MG2_CarrierResponseToVarianceRequest.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_CarrierResponseToVarianceRequest);
                    }
                }

                propinfo = null;
                propinfo = objMG2.MG2_CarrierResponseToVarianceRequest.MG2_CarrierUndertaken.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_CarrierResponseToVarianceRequest.MG2_CarrierUndertaken);
                    }
                }

                propinfo = null;
                propinfo = objMG2.MG2_CarrierResponseToVarianceRequest.CarrierResponseToVarianceRequestValue.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_CarrierResponseToVarianceRequest.CarrierResponseToVarianceRequestValue);
                    }
                }

                propinfo = null;
                propinfo = objMG2.MG2_DenialInfromallyResolvedInProviderAndCarrier.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_DenialInfromallyResolvedInProviderAndCarrier);
                    }
                }

                propinfo = null;
                propinfo = objMG2.MG2_RequestForReviewOfDenial.GetType().GetProperties();

                if (propinfo != null && propinfo.Length > 0)
                {
                    foreach (PropertyInfo prop in propinfo)
                    {
                        SetData(FormPdfDoc, prop, objMG2.MG2_RequestForReviewOfDenial);
                    }
                }
        ...
        private bool SetData(PDFDoc oDoc, PropertyInfo prop, object objClass)
        {
            ...
            try
            {
                ...
                sField = objMG2.GetAttributeDisplayName(prop);
        ...
        {
            gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.NYWCForms, gloAuditTrail.ActivityCategory.NYWCForms, gloAuditTrail.ActivityType.Close, "MG-2 Form Closed.", PatientId, iFormID, 0, gloAuditTrail.ActivityOutCome.Success, (_messageBoxCaption == "gloPM" ? gloAuditTrail.SoftwareComponent.gloPM : gloAuditTrail.SoftwareComponent.gloEMR));
            this.Close();
        }
        ...
        private void tsb_Print_Click(object sender, EventArgs e)
        {
            try
            {
                if (iFormID <= 0)
                {
                    MessageBox.Show("MG-2 Form you are trying to print is not yet saved." + System.Environment.NewLine + "Form will be saved automatically before printing.", _messageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                //else
                //{
                    gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.NYWCForms, gloAuditTrail.ActivityCategory.NYWCForms, gloAuditTrail.ActivityType.Print, "MG-2 Form printed from system.", PatientId, iFormID, 0, gloAuditTrail.ActivityOutCome.Success, (_messageBoxCaption == "gloPM" ? gloAuditTrail.SoftwareComponent.gloPM : gloAuditTrail.SoftwareComponent.gloEMR));
                //}
   ...
   //     pdftron.PDF.Rect rect = null;
        ...
        //}
        private  void SetDataToMG2Object(DataTable dtData, ref MG2 objMG2)
        {
            ...
            try
            {
                foreach (DataRow drRow in dtData.Rows)
                {
                    ...
                    prop = objMG2.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        if (!string.IsNullOrEmpty(System.Convert.ToString(oValue)))
                        {
                            ...
                            prop.SetValue(objMG2, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_CaseDetails.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_CaseDetails, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_PatientDetails.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_PatientDetails, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_AttendingDoctorInformation.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_AttendingDoctorInformation, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_ApprovalRequestVarianceFor.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_ApprovalRequestVarianceFor, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_HeaderInfo.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_HeaderInfo, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_IndependentMedicalExaminer.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_IndependentMedicalExaminer, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_CarrierResponseToVarianceRequest.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_CarrierResponseToVarianceRequest, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_CarrierResponseToVarianceRequest.MG2_CarrierUndertaken.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_CarrierResponseToVarianceRequest.MG2_CarrierUndertaken, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_CarrierResponseToVarianceRequest.CarrierResponseToVarianceRequestValue.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_CarrierResponseToVarianceRequest.CarrierResponseToVarianceRequestValue, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_DenialInfromallyResolvedInProviderAndCarrier.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_DenialInfromallyResolvedInProviderAndCarrier, oValue, null);
                        }

                        continue;
                    }

                    prop = objMG2.MG2_RequestForReviewOfDenial.GetType().GetProperty(System.Convert.ToString(drRow["FieldName"]));
                    ...
                    if (prop != null)
                    {
                        ...
                        {
                            oValue = System.Convert.ChangeType(oValue, prop.PropertyType);
                            prop.SetValue(objMG2.MG2_RequestForReviewOfDenial, oValue, null);
                        }
        ...
        private Boolean SetDataToDBAdapter()
        {
            ...
            try
            {
                ...
                if (dsWorkersCompForms.Tables["WorkersCompForms_MST"].Rows.Count <= 0)
                {
                    ...
                    drRow["FormType"] =(int)WorkersCompFormTypes.MG2;
                ...
                for (itr = FormPdfDoc.GetFieldIterator(); itr.HasNext(); itr.Next())
                    {
                        ...
                        switch (type)
                        {
                            case Field.Type.e_button:
                                ...
                                {
                                    drRow[0]["FormFieldValueImage"] =ObjWorkerComp.GetProviderSignImage(field,ref FormPdfDoc,1,_messageBoxCaption);
                                }
        ...
        private DataTable GetAllPropertiesInfo(MG2 objMG2)
        {
            ...
            try
            {
                ...
                propinfo = null;
                propinfo = objMG2.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2);

                propinfo = null;
                propinfo = objMG2.MG2_CaseDetails.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_CaseDetails);

                propinfo = null;
                propinfo = objMG2.MG2_PatientDetails.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_PatientDetails);

                propinfo = null;
                propinfo = objMG2.MG2_AttendingDoctorInformation.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_AttendingDoctorInformation);

                propinfo = null;
                propinfo = objMG2.MG2_ApprovalRequestVarianceFor.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_ApprovalRequestVarianceFor);

                propinfo = null;
                propinfo = objMG2.MG2_HeaderInfo.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_HeaderInfo);

                propinfo = null;
                propinfo = objMG2.MG2_IndependentMedicalExaminer.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_IndependentMedicalExaminer);

                propinfo = null;
                propinfo = objMG2.MG2_CarrierResponseToVarianceRequest.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_CarrierResponseToVarianceRequest);

                propinfo = null;
                propinfo = objMG2.MG2_CarrierResponseToVarianceRequest.MG2_CarrierUndertaken.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_CarrierResponseToVarianceRequest.MG2_CarrierUndertaken);

                propinfo = null;
                propinfo = objMG2.MG2_CarrierResponseToVarianceRequest.CarrierResponseToVarianceRequestValue.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_CarrierResponseToVarianceRequest.CarrierResponseToVarianceRequestValue);

                propinfo = null;
                propinfo = objMG2.MG2_DenialInfromallyResolvedInProviderAndCarrier.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_DenialInfromallyResolvedInProviderAndCarrier);

                propinfo = null;
                propinfo = objMG2.MG2_RequestForReviewOfDenial.GetType().GetProperties();
                PopulatePropertiesData(ref dtProp, propinfo, objMG2.MG2_RequestForReviewOfDenial);
        ...
        private void PopulatePropertiesData(ref DataTable dtProp, PropertyInfo[] propinfo, object obj)
        {
            ...
            try
            {
                ...
                { 
                    ...
                    foreach (PropertyInfo prop in propinfo)
                    {
                        string sDisplayName = objMG2.GetAttributeDisplayName(prop);
                ...
                if (bIsSuccess)
                {
                    ...
                    gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.NYWCForms, gloAuditTrail.ActivityCategory.NYWCForms,
                        gloAuditTrail.ActivityType.Save,
                         (lblClaimNo.Text.Trim().Length > 0) ? "MG-2 Form saved with claim '" + lblClaimNo.Text + "'." : "MG-2 Form saved without claim association.",
                ...
                if (IsValidClaimNumber())
                {
                   ofrmWCPatientClaims = new frmWCPatientClaims(PatientId);
                        ...
                        dsInfo = ObjWorkerComp.GetWorkersCompPredefinedInfo(PatientId, TransactionID, TransactionMasterID, VisitId, ClinicId);
                         ...
                         ObjWorkerComp.SetMG2PrdefinedInfo(dsInfo, ref objMG2,true,false);

                         PDFDoc FreshPdfdoc = new PDFDoc(global::gloBilling.Properties.Resources.MG2_tab_ImageSign, global::gloBilling.Properties.Resources.MG2_tab_ImageSign.Length);
                         try
                         {
                             FreshPdfdoc.InitSecurityHandler();
                         }
                         ...
                         gloAuditTrail.gloAuditTrail.CreateAuditLog(gloAuditTrail.ActivityModule.NYWCForms, gloAuditTrail.ActivityCategory.NYWCForms, gloAuditTrail.ActivityType.Modify, "Claim modified on MG-2 Form with new value '" + lblClaimNo.Text + "'.", PatientId, iFormID, 0, gloAuditTrail.ActivityOutCome.Success, (_messageBoxCaption == "gloPM" ? gloAuditTrail.SoftwareComponent.gloPM : gloAuditTrail.SoftwareComponent.gloEMR));
        ...
        private void LoadPDFSign(ref PDFDoc SignPdfdoc)
        {
            try
            {
                string fname = ObjWorkerComp.SignPDFImage(ref SignPdfdoc, objMG2.MG2_ApprovalRequestVarianceFor.ProviderSignImage, "topmostSubform[0].Page1[0].ProviderSignature[0]", 1);
        ...
        private void frmWorkerCompFormViewer_MG2_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void frmWorkerCompFormViewer_MG2_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (_pdfview != null)
