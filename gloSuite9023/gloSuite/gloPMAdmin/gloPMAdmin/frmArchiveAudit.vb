...
#End Region
    ...
    Private Sub Fill_ActivityTypesUsers()
        ...
        clAudit = objAudit.Fill_Users
        cmbUsers.Items.Clear()
        cmbUsers.Items.Add("All")
        For nCount = 1 To clAudit.Count
            cmbUsers.Items.Add(clAudit.Item(nCount))
        Next
    ...
    Private Sub btnPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPreview.Click
        ...
        Try
            ...
            If Trim(txtPatient.Text) = "" Then
                dtAuditReports = objAuditReports.RetrieveAuditReports(dtFrom.Value.Date, dtTo.Value.Date, cmbCategory.SelectedItem, cmbUsers.SelectedItem)
            Else
                ...
                Else
                    If Trim(cmbPatientType.Text) = "First Name" Then
                        dtAuditReports = objAuditReports.RetrieveAuditReports(dtFrom.Value.Date, dtTo.Value.Date, cmbCategory.SelectedItem, cmbUsers.SelectedItem, , Trim(txtPatient.Text))
                    ElseIf Trim(cmbPatientType.Text) = "Last Name" Then
                        dtAuditReports = objAuditReports.RetrieveAuditReports(dtFrom.Value.Date, dtTo.Value.Date, cmbCategory.SelectedItem, cmbUsers.SelectedItem, , , Trim(txtPatient.Text))
                    Else
                        dtAuditReports = objAuditReports.RetrieveAuditReports(dtFrom.Value.Date, dtTo.Value.Date, cmbCategory.SelectedItem, cmbUsers.SelectedItem, , txtPatient.Text)
                    End If
                End If
            End If
            ...
            grdTableStyle.GridColumnStyles.AddRange(New DataGridColumnStyle() {grdColAuditReportID, grdColActivityDate, grdColActivityCategory, grdColDescription, grdColPatientCode, grdColPatient, grdColUser, grdColMachineName, grdColSoftwareComponent, grdColOutcome})
            dgData.TableStyles.Clear()
            dgData.TableStyles.Add(grdTableStyle)

            Dim _logText As String = ""

            objAudit.CreateLog(clsAudit.enmActivityType.Query, "The user queried and viewed the data to be archived from " & dtFrom.Value.Date & " to " & dtTo.Value.Date & " for the Category " & cmbCategory.SelectedItem & " for the user " & cmbUsers.SelectedItem, gstrLoginName, gstrClientMachineName, 0, , clsAudit.enmOutcome.Success)

            Me.Cursor = Cursors.Default
        Catch objErr As Exception
            Me.Cursor = Cursors.Default
            MessageBox.Show(objErr.ToString, gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Error)
            objAudit.CreateLog(clsAudit.enmActivityType.Query, "Error while viewing the data to be archived from" & dtFrom.Value.Date & " to " & dtTo.Value.Date & " for the Category " & cmbCategory.SelectedItem & " for the user " & cmbUsers.SelectedItem, gstrLoginName, gstrClientMachineName, 0, , clsAudit.enmOutcome.Success)
        Finally
            objAudit = Nothing
        End Try
    ...
    Private Sub btnArchive_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnArchive.Click
        ...
        Try
            If MessageBox.Show("Are you sure, you want to archive the audit trail data?", gstrMessageBoxCaption, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Me.Cursor = Cursors.WaitCursor

                If optAll.Checked = True Then
                    If objAudit.ArchiveAuditReports() = True Then
                        Me.Cursor = Cursors.Default
                ...
                Else
                    If Trim(txtPatient.Text) = "" Then
                        If objAudit.ArchiveAuditReports(dtFrom.Value.Date, dtTo.Value.Date, cmbCategory.SelectedItem, cmbUsers.SelectedItem) = True Then
                            Me.Cursor = Cursors.Default
                            MessageBox.Show("AuditTrail data has been successfully archived", gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Information)
                            'sarika  22nd feb
                            objAudit.CreateLog(clsAudit.enmActivityType.Other, gstrLoginName & " user has successfully archived AuditTrail data from " & dtFrom.Value.Date & " to " & dtTo.Value.Date & " for the Category " & cmbCategory.SelectedItem & " for the user " & cmbUsers.SelectedItem, gstrLoginName, gstrClientMachineName, 0, True)
                            '   objAudit = Nothing
                            Me.Close()
                            Exit Sub
                        End If
                    Else
                        If Trim(cmbPatientType.Text) = "Patient ID" Then
                            If IsNumeric(txtPatient.Text) = True Then
                                If objAudit.ArchiveAuditReports(dtFrom.Value.Date, dtTo.Value.Date, cmbCategory.SelectedItem, cmbUsers.SelectedItem, CInt(txtPatient.Text)) = True Then
                                    Me.Cursor = Cursors.Default
                                    MessageBox.Show("AuditTrail data has been successfully archived", gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Information)
                                    'sarika  22nd feb
                                    objAudit.CreateLog(clsAudit.enmActivityType.Other, gstrLoginName & " user has successfully archived AuditTrail data from " & dtFrom.Value.Date & " to " & dtTo.Value.Date & " for the Category " & cmbCategory.SelectedItem & " for the user " & cmbUsers.SelectedItem, gstrLoginName, gstrClientMachineName, 0, True)
                        ...
                        ElseIf Trim(cmbPatientType.Text) = "First Name" Then
                            If objAudit.ArchiveAuditReports(dtFrom.Value.Date, dtTo.Value.Date, cmbCategory.SelectedItem, cmbUsers.SelectedItem, , txtPatient.Text) = True Then
                                Me.Cursor = Cursors.Default
                                MessageBox.Show("AuditTrail data has been successfully archived", gstrMessageBoxCaption, MessageBoxButtons.OK, MessageBoxIcon.Information)
                                'sarika  22nd feb
                                objAudit.CreateLog(clsAudit.enmActivityType.Other, gstrLoginName & " user has successfully archived AuditTrail data from " & dtFrom.Value.Date & " to " & dtTo.Value.Date & " for the Category " & cmbCategory.SelectedItem & " for the user " & cmbUsers.SelectedItem, gstrLoginName, gstrClientMachineName, 0, True)
                ...
                objAudit.CreateLog(clsAudit.enmActivityType.Other, "Unable to archive auditTrail data from " & dtFrom.Value.Date & " to " & dtTo.Value.Date & " for the Category " & cmbCategory.SelectedItem & " for the user " & cmbUsers.SelectedItem, gstrLoginName, gstrClientMachineName, 0, , clsAudit.enmOutcome.Failure)
