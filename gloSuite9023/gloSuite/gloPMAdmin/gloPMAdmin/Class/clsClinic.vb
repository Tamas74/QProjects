...
#Region " Public Functions"
    Public Sub sp_ScanClinic()
        ...
        If objSQLDataReader.HasRows = True Then
            ...
            If IsDBNull(objSQLDataReader.Item("Country")) = False Then      ''Country
                _sCountry = objSQLDataReader.Item("Country")
            Else
                _sCountry = ""
            End If
            If IsDBNull(objSQLDataReader.Item("County")) = False Then       ''County
    ...
    End Function
    Public Function RetrieveClinicIDQualifiers(ByVal nClinicID As Int64) As DataTable
        Dim objCon As New SqlConnection
        objCon.ConnectionString = gloPMAdmin.mdlGeneral.GetConnectionString()
        Dim objCmd As New SqlCommand
        objCmd.CommandType = CommandType.Text
        objCmd.CommandText = "select nQualifierID,sValue from Clinic_ID_Qualifiers where nClinicID=" + Convert.ToString(nClinicID)
        objCmd.Connection = objCon
        objCmd.CommandTimeout = 0
        objCon.Open()
        Dim objDA As New SqlDataAdapter(objCmd)
    ...
    Public Function CheckColumnExists() As Boolean
        ...
        conn = New SqlConnection(gloPMAdmin.mdlGeneral.GetConnectionString())
