...
Public Class ClsICD10Settings
    ...
    Public Function FetchICDDOS() As String
        ...
        Return _Result
    End Function
    Public Function SaveData(ByVal _List As IEnumerable)
        Dim olist1 As List(Of BL_ICD10TransitionSetting) = Nothing
        Try
            olist1 = (From Result In _List
                     Select New BL_ICD10TransitionSetting With {.nContactID = Result.nContactID,
                                                                .nClinicID = 1,
                                                                .dtDOSDate = Result.dtDOSDate,
                                                                .dtCreatedDate = DateTime.Now,
                                                                .dtModifiedDate = DateTime.Now,
                                                                .nID = 0}).ToList()
            oDataContext.ExecuteCommand("Truncate Table BL_ICD10TransitionSettings")
            oDataContext.BL_ICD10TransitionSettings.InsertAllOnSubmit(olist1)
            oDataContext.SubmitChanges()
        Catch ex As Exception
            Throw ex
        Finally
            _List = Nothing
            If Not olist1 Is Nothing Then
                olist1.Clear()
                olist1 = Nothing
            End If
        End Try
    End Function
    Public Function SaveData(ByVal oList As List(Of BL_ICD10TransitionSetting))
        Try
            oDataContext.ExecuteCommand("Truncate Table BL_ICD10TransitionSettings")
            oDataContext.BL_ICD10TransitionSettings.InsertAllOnSubmit(oList)
            oDataContext.SubmitChanges()
        Catch ex As Exception
            Throw ex
        Finally
    ...
    Public Function getChangeDataforAuditing()
        ...
        End Try
    End Function
    Private Sub WriteAuditLog(_List As IEnumerable(Of BL_ICD10TransitionSetting), ByVal Type As clsAudit.enmActivityType)
